...
//									continue;
		...
		if (CountOfNearest)
		{
			...
			{
				FILE *f = fopen("c:\\markcheck.txt", "a+t");
				fprintf(f, "%d %f %f %f %f %f %f %f %f\n", CurrentMark, Expected.X, Expected.Y, Expected.Z, PosX, PosY, PosZ,
					pM->pMarks[CurrentMark].Nominal.X, pM->pMarks[CurrentMark].Nominal.Y);
				fclose(f);				
				};
			if (TimedOut || Limiter)
			{
		...
		{
			if (!EchoMessageAsk("Scanning finished.\nExit?", "HELP REQUEST"))
			{
