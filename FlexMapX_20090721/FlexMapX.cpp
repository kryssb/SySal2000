...
STDAPI DllRegisterServer(void)
{
	...
	try
	{
		...
		MyKey.Close();
		if (MyKey.Create(HKEY_LOCAL_MACHINE, "Software\\SySal2\\Classes\\FlexibleMapX", REG_NONE, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE) != ERROR_SUCCESS) throw 3;
		MyKey.Close();
		MyKey.Open(HKEY_LOCAL_MACHINE, "Software\\SySal2\\Classes\\FlexibleMapX", KEY_READ | KEY_WRITE);
		char CLSIDString[64];
...
//GS deprecated		MyKey.SetValue(CLSIDString, "CLSID");
      MyKey.SetStringValue( "CLSID", CLSIDString );
		MyKey.Close();
		}
	catch (...)
	{
		...
		MyKey.RecurseDeleteKey("Software\\SySal2\\Classes\\FlexibleMapX");
		MyKey.Close();
		};
