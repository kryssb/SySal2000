...
void CXYSpeed::UseJoystick(bool usejoystick)
{
	if (pGamePad) delete pGamePad;
	if (usejoystick)
	{
		pGamePad = new CImage();
		pGamePad->LoadFromResource(AfxGetResourceHandle(), MAKEINTRESOURCE(IDB_GAMEPAD));
	}
	else pGamePad = 0;
}
...
void CXYSpeed::OnPaint() 
{
	...
	if (IsWindowEnabled())
	{
		...
		if (pGamePad)
			pGamePad->Draw(dc.m_hDC, 0, 0, MyRect.Width(), MyRect.Height());		
		else
		{
