...
void CTracker::ApplyDistortionCorrection(SDistortion Dist,SEmulsionSpace &Space)
{
...
void CTracker::FindGlobalDistortion(SDistortion &Dist)
{
	...
	int count = CountTracks(m_pTrackUp);
	SMip * pMip;
			...
			for(B = Bmin; B < Bmax; B += DB)
			{
				for(x0 = xmin; x0 <= xmax; x0 += DX)
				{
					for(y0 = ymin; y0 <= ymax; y0 += DY)
					{
						...
						{
							pMip = m_pTrackUp;weight = 0;WMD = 0;
	...
	{
		SEmulsionSpace &Space = *TP.pEmSpaceUp;
		SIntGrain *pGrains = Space.pGrains;
