...
#else
#error ERROR: __IM_IRQIO_H__ TBD for _IM_COMP_MICROSOFT
#endif
...
#if defined(_IM_HOST_OS_NT) || defined(_IM_HOST_OS_QNX4)
#if defined(_IM_HOST_OS_NT_KERNEL)
...
typedef long (IM_FTYPE IM_PTYPE *GENESISHOOKFCTPTR)(PGENESIS_HOOK_LIST pHookList);

#if defined(_IM_HOST_OS_QNX4)
struct QNX_EVENT
{
    long autoCreate;
    sem_t     event;
...
struct _GENESIS_HOOK_FUNCTION
{
   ...
   void                         *UserDataPtr;
#if defined(_IM_HOST_OS_DOS) || defined(_IM_HOST_OS_QNX4)
   long                          hEventUsr;
#elif defined(_IM_HOST_OS_NT)
   HANDLE                        hEventUsr;
#endif
...
IM_EXTC void IM_FTYPE irq_calloldhandler(int irqnr);
IM_EXTC void IM_FTYPE irq_disable(int irqnr);
IM_EXTC void IM_FTYPE irq_enable(int irqnr);
IM_EXTC int  IM_FTYPE irq_sethandler(int irqnr, void (*handler)(int), void IM_PTYPE *stack);
IM_EXTC int  IM_FTYPE irq_unsethandler(int irqnr);

#elif defined(_IM_HOST_OS_NT) && !defined(_IM_HOST_OS_NT_KERNEL) || (defined(_IM_HOST_OS_QNX4) && !defined(_IM_HOST_OS_QNX4_DRIVER))

IM_EXTC HANDLE IM_FTYPE GenesisGetHandleEvent(PGENESIS_HOOK_FUNCTION pGenesisHookFunction);

#endif

#if !defined(_IM_HOST_OS_NT_KERNEL)

IM_EXTC PGENESIS_HOOK_LIST IM_FTYPE GenesisGetHookList(long NodeId, long InterruptType);
IM_EXTC PGENESIS_HOOK_FUNCTION IM_FTYPE GenesisGetNextHookFunction(PGENESIS_HOOK_LIST pHookList, PGENESIS_HOOK_FUNCTION *ppGenesisHookFunction);
IM_EXTC GENESISHOOKFCTPTR IM_FTYPE GenesisGetFuncPtr(PGENESIS_HOOK_FUNCTION pGenesisHookFunction);
IM_EXTC void IM_PTYPE * IM_FTYPE GenesisGetUsrDataPtr(PGENESIS_HOOK_FUNCTION pGenesisHookFunction);
IM_EXTC long IM_FTYPE GenesisGetInterruptType(PGENESIS_HOOK_FUNCTION pGenesisHookFunction);
IM_EXTC long IM_FTYPE GenesisGetHookInterruptType(PGENESIS_HOOK_LIST pHookList);
IM_EXTC long IM_FTYPE GenesisHookInterruptFunction(long NodeId, GENESISHOOKFCTPTR InterruptFunctionPtr, unsigned long InterruptType, void IM_PTYPE *phEvent, void IM_PTYPE *UserDataPtr);
