...
void CTestConfig::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_TS_CENTERCIRCLES, m_ShowCenterCircles);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CTestConfig, CDialog)
	...
	ON_BN_CLICKED(IDC_TS_CENTERCIRCLES, OnCenterCircles)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CTestConfig::OnCenterCircles() 
{
	// TODO: Add your control notification handler code here
	UpdateData();
	int R;
	int C;
	G.GetColorCode(m_ShowCenterCircles ? VC_DKBLUE : VC_TRANSPARENT, &C);
	for (R = 16; R < ImW / 2 && R < ImH / 2; R += 64)
		G.Arc(ImW / 2, ImH / 2, R, R, 0, 360, C);
}
