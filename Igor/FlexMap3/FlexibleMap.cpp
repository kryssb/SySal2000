...
STDMETHODIMP CFlexibleMap::GetClassName(ConnectionName name)
{
	...
	strncpy(name, "Flexible Sheet Map 3", SYSAL_MAX_CONN_NAME_LEN);
	return S_OK;
}
...
//N V xmin ymin xmax ymax Nbits Dmark
		...
		//where N- number of label (used by caller, not here) V - is label version, x,y - coordinates of outermost marks, Nbits number of bits
...
//					MessageBox((HWND)hWnd, "The label string was not correctly formed!", "Formatting Error", MB_ICONERROR | MB_OK);
//					throw 3;
//					}
if(!Valid) {
	...
	try
	{
		...
		if (pIOMap->Hdr.Hdr.HeaderFormat == MAP_MARKEXT_HEADER_TYPE)
		{
			for (i = 0; i < pMap->Count; i++)
			{
				...
				pMap->pMarks[i].Status = (pIOMap->Map2.pMarks[i].Status == MARK_STATUS_NOTFOUND) ? MARK_STATUS_NOTFOUND : MARK_STATUS_NOTSEARCHED;
				};
			}
		else
		{
...
*/
	...
	if (pMap)
	{
		...
		Map = *pMap;
			...
			Valid = true;
	// Now check if we need to read labels//
	const char separators[] = "\r\n;";
	char *replica = new char[strlen(labexts) + 1];
		strcpy(replica, labexts);
		char *line;
		char mess[128];
        int labelI=-1;
		int label;
		...
		{
			if(lb==0) line=strtok(replica, separators); else line=strtok(NULL, separators);
            if(!line) throw 10;
			if(sscanf(line,"%d",&labelI)!=1) throw 11;
			if( (labelI<0) || (labelI>3) ) throw 12;
			ReadLabels((BYTE*)line, hWnd, hNotifyStatusChangeEvent,&label);
			if(label==-1) throw 13;
			if(pIOMap->Hdr.Id.Part[labelI]!=label) throw 14;
			}
    // end of labels check
	...
	catch (...)
	{
		...
		if (labexts) { delete [] labexts; labexts=0; }
		Valid = false;
...
IO_MarkMap * CFlexibleMap::ReadMapFromString(char *str)
{
	...
	try
	{
		...
		if (sscanf(line, "%d", &marks) != 1) throw 1;
		pMap = (IO_MarkMap *)CoTaskMemAlloc(sizeof(IO_MarkMap) + marks * sizeof(Mark2));
		if (!pMap) throw 2;
		pMap->Hdr.Hdr.InfoType = MAP_HEADER_BYTES | MAP_FULL_SECTION;
		pMap->Hdr.Hdr.HeaderFormat = MAP_MARK_HEADER_TYPE;
		pMap->Hdr.Id = Id;
		pMap->Map.Count = marks;
		pMap->Map.pMarks = (Mark *)(void *)((char *)&pMap->Map.pMarks + sizeof(int));
		marks = 0;
		while ((marks < pMap->Map.Count) && (line = strtok(0, separators)))
		{
			if (sscanf(line, "%d %f %f %f %f %d %d", &pMap->Map.pMarks[marks].Id, &pMap->Map.pMarks[marks].Nominal.X, &pMap->Map.pMarks[marks].Nominal.Y,
				&pMap->Map.pMarks[marks].Stage.X, &pMap->Map.pMarks[marks].Stage.Y, &pMap->Map.pMarks[marks].Status, &pMap->Map.pMarks[marks].Status) != 7) throw 3;
			marks++;
			}
		if (marks < pMap->Map.Count) throw 4;
		}
	catch (...)
	{
		if (pMap) 
...
IO_MarkMap * CFlexibleMap::ReadMapExtFromString(char *str)
{
	const char separators[] = "\r\n;";
	char *replica = new char[strlen(str) + 1];
	IO_MarkMap *pMap = 0;
	try
	{
		...
		if (!pMap) throw 2;
		pMap->Hdr.Hdr.InfoType = MAP_HEADER_BYTES | MAP_FULL_SECTION;
		pMap->Hdr.Hdr.HeaderFormat = MAP_MARKEXT_HEADER_TYPE;
		pMap->Hdr.Id = Id;
		pMap->Map2.Count = marks;
		pMap->Map2.pMarks = (Mark2 *)(void *)((char *)pMap + sizeof(IO_MarkMap));
		marks = 0;		
		pMap->Map2.PlateMinX = tminx;
		pMap->Map2.PlateMinY = tminy;
		pMap->Map2.PlateMaxX = tmaxx;
		pMap->Map2.PlateMaxY = tmaxy;
		while ((marks < pMap->Map2.Count) && (line = strtok(0, separators)))
		{
			if (sscanf(line, "%d %f %f %f %f %d %d %d", &pMap->Map2.pMarks[marks].Id, &pMap->Map2.pMarks[marks].Nominal.X, &pMap->Map2.pMarks[marks].Nominal.Y,
				&pMap->Map2.pMarks[marks].Stage.X, &pMap->Map2.pMarks[marks].Stage.Y, &pMap->Map2.pMarks[marks].Status, &pMap->Map2.pMarks[marks].Status,
				&pMap->Map2.pMarks[marks].Side) != 8) throw 3;
			marks++;
			}
		if (marks < pMap->Map2.Count) throw 4;
		}
	catch (...)
	{
		if (pMap) 
 ...
 IO_MarkMap * CFlexibleMap::ReadMapLabExtFromString(char *str, char *labexts, int *Nlabels)
{
	const char separators[] = "\r\n;";
	char *replica = new char[strlen(str) + 1];
	IO_MarkMap *pMap = 0;
	try
	{
		...
		*Nlabels=0;
		...
		line = strtok(0, separators);
		float tminx, tminy, tmaxx, tmaxy;
		if (sscanf(line, "%d %f %f %f %f %d", &marks, &tminx, &tminy, &tmaxx, &tmaxy, &labels) != 6) throw 1;
        *Nlabels=labels;
		pMap = (IO_MarkMap *)CoTaskMemAlloc(sizeof(IO_MarkMap) + marks * sizeof(Mark2));
		if (!pMap) throw 2;
		pMap->Hdr.Hdr.InfoType = MAP_HEADER_BYTES | MAP_FULL_SECTION;
        ...
        labels=0;
		labexts[0]=0;
		while( (labels < *Nlabels) && (line = strtok(0, separators)) ) 
		{
			sprintf(labexts,"%s%s; ",labexts,line);
			labels++;
			}
		if( labels < *Nlabels) throw 5;
...
float CFlexibleMap::CalculateAverageInCircle(BYTE *pImage, int W, int H, int iXC, int iYC, int Diam)
{
//  char mess[256];
int index;
...
float Summ=0;
int Np=0;  
//sprintf(mess,"Circle at %d,%d, diameter %d pixels",iXC,iYC,Diam);
// AfxMessageBox(mess);

for(int y=ymin;y<ymax;y++) for(int x=xmin;x<xmax;x++) 
  if(hypot(iXC-x,iYC-y)<(Diam/2))
  {
  index=y*W+x;
  Np++;
  Summ+=pImage[index];
  }
//  sprintf(mess,"%d pixels in circle at %d %d with diameter %d",Np,iXC,iYC,Diam);
//  AfxMessageBox(mess);
  if(Np>0) return Summ/Np;
  return -1;
}
