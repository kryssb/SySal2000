...
UINT CInitMap::ExecThread()
{
	...
	MarkStatusChanged = false;	
	PostMessage(UM_CHANGE_STATUS, MSG_SCAN_THREAD_CONTINUING);
	VPLock();
	...
	do
	{
		...
		MarkStatusChanged = false;
		float OrigMark0StageX; //3 lines by Kreslo I.
		float OrigMark0StageY; //
		int Mark0Index;        //
		switch (CountOfNearest)
		{
			case 0:		
						...
						//			Expected.Y = pM->PlateMaxY - pM->pMarks[0].Nominal.Y + CornerY;
								...
								OrigMark0StageX=pM->pMarks[CurrentMark].Stage.X; //3 lines by Kreslo I.
								OrigMark0StageY=pM->pMarks[CurrentMark].Stage.Y; //
								Mark0Index=CurrentMark;								//
...
//									continue;
		...
		*/	case 3:	/*	{
                ...
                */                // commented cases 1-3 : Kreslko I., added next clause:
							{

							if (!F.SheetIsRotatedBy180Deg)
							{
								Expected.X = pM->pMarks[Mark0Index].Stage.X + pM->pMarks[CurrentMark].Stage.X - OrigMark0StageX;
								Expected.Y = pM->pMarks[Mark0Index].Stage.Y + pM->pMarks[CurrentMark].Stage.Y - OrigMark0StageY;
								}
							else
							{
								Expected.X = pM->pMarks[Mark0Index].Stage.X - pM->pMarks[CurrentMark].Stage.X + OrigMark0StageX;
...
//     iSt->Stop(2);
			...
			iSt->PosMove(2, Expected.Z + F.FocusStroke, F.ZSetSpeed, Z_ACC);
			float NextTime = MaxTime * 1.f + GetTickCount();
			bool TimedOut, Arrived, Limiter;
//AfxMessageBox("Entering the loop");
...
//     iSt->Stop(2);
				...
				if(TimedOut) 
					...
					iSt->Stop(1);
					iSt->Stop(2);
					NextTime = MaxTime * 1.f + GetTickCount();
//					AfxMessageBox("Timeout condition! Restarting.");
							iSt->PosMove(0, Expected.X, F.LongDistSpeed, F.LongDistAcc);
							iSt->PosMove(1, Expected.Y, F.LongDistSpeed, F.LongDistAcc);
							iSt->PosMove(2, Expected.Z + F.FocusStroke, F.ZSetSpeed, Z_ACC);
					}
...
//			while (!TimedOut && !Arrived && !Limiter);
			while ( !Arrived && !Limiter);

//AfxMessageBox("Loop is over");
//			if (TimedOut || Limiter)
			if ( Limiter)
			{
				pM->pMarks[CurrentMark].Status = MARK_STATUS_NOTFOUND;
			...
			else
			{
				...
				FM.SetStep(fabs(CAM.WinWidth * CAM.PixelToMicronX) - 2*F.Diameter, fabs(CAM.WinHeight * CAM.PixelToMicronY) - F.Diameter);
				do
				{
					float X, Y;
...
//     iSt->Stop(2);
		...
		{			
			...
			if (goodmarks < 3)
			{
				if (!AutoFirstMark && !EchoMessageAsk("At least 3 good marks are required\r\nMap will be invalid if you exit now\r\nExit?", "HELP REQUEST"))
				{
					CurrentMark = MarkShown;
					Pause = true;
					PostMessage(UM_CHANGE_STATUS, MSG_SCAN_THREAD_PAUSED);
					if (PauseAndCheckTermination()) return Exit();				
					if (MarkStatusChanged)
					{
///????????
...
///????????
	...
	for (i = goodmarks = 0; i < pM->Count; i++)
		if (pM->pMarks[i].Status == MARK_STATUS_FOUND)
			goodmarks++;	
	PostMessage(UM_CHANGE_STATUS, (goodmarks < 3) ? MSG_SCAN_THREAD_FAILURE : MSG_SCAN_THREAD_SUCCESS);
