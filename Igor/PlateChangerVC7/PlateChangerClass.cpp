...
CPlateChangerClass::CPlateChangerClass()
{
	...
	strcpy(Slots[2].Name, "FrameGrabber");
	int i;
...
//		{ if (Slots[0].pUnknown->QueryInterface(IID_IStage2, (void **)&iSt) != S_OK) iSt=0;	}		
		...
		if (Slots[2].pUnknown)
		{
			if (Slots[2].pUnknown->QueryInterface(IID_IFrameGrabber, (void **)&iFG) != S_OK) iFG->Release();
			if (Slots[2].pUnknown->QueryInterface(IID_IAutomaticLightControl, (void **)&iLC) != S_OK) iLC->Release();
			}
		else { iFG=0; iLC=0;}
