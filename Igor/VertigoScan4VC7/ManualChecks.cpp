...
void CManualChecks::ShowTrack(float CurrX, float CurrY, bool bShow)
{
	...
	if (bShow)
	{
		...
		iSM->DirMapPoint(m_IX + m_SX * (C.VStep(true) - m_IZ), m_IY + m_SY * (C.VStep(true) - m_IZ), &TopExt.X, &TopExt.Y);
		...
		iSM->DirMapPoint(m_IX + m_SX * (- C.BaseThickness - C.VStep(false) - m_IZ), m_IY + m_SY * (- C.BaseThickness - C.VStep(false) - m_IZ), &BottomExt.X, &BottomExt.Y);
