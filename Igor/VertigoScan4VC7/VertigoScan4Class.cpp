...
bool CVertigoScan4Class::ExecLoadPlate(float *pAutoZReset, IStage *iSt, IPlateChanger *iPc, Identifier id, char *textid)
{
...
bool CVertigoScan4Class::ExecUnloadPlate(IStage *iSt, IPlateChanger *iPc)
{
	ISySalObject *iStOb = 0;
	iSt->QueryInterface(IID_ISySalObject, (void **)&iStOb);
	iSt->Stop(0);
...
bool CVertigoScan4Class::ExecSetParam(char *objname, char *paramname, char *paramvalue)
{
    ...
    if (!stricmp("VertigoScanner", objname)){ //if configure self
				SySalConfig T;
				T.CountOfItems = 0;
				T.Name[0] = 0;
				T.pItems = 0;
