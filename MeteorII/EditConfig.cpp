...
CEditConfig::CEditConfig(CMeteorII &m, CWnd* pParent /*=NULL*/)
	: CDialog(CEditConfig::IDD, pParent), M(m)
{
...
void CEditConfig::OnTest() 
{
	...
	if (M.SetConfig(&Temp) != S_OK)	MessageBox("Can't apply temporary configuration for test.\r\nAborting.", "Internal Error", MB_ICONERROR);
	else
	{
		CTestConfig MyDlg(M, this);
		MyDlg.DoModal();
		};
	
	if (M.SetConfig(&Saved) != S_OK) MessageBox("Can't restore previous configuration.\r\nModule functionality might be compromised.\r\nPlease try destroying and recreating object.", "Internal Error", MB_ICONERROR);
