...
void COptimizeProgramDlg::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if (nIDCtl == IDC_PO_DISPLAY)
	{
		...
		CFont LabelFont;
		LabelFont.CreatePointFont(8, "Arial", &dc);
		...
		{
			float x, y, v;
			dc.MoveTo(x = (lpDrawItemStruct->rcItem.left + GRAPH_XMARGIN + GRAPH_XLABELSPACE + GRAPH_TICKSIZE + XScale * ((v = XMinTick + i * XTicks) - MinX)), 
				y = lpDrawItemStruct->rcItem.bottom - GRAPH_YMARGIN - GRAPH_YLABELSPACE);
		...
		for (i = 0; i < XTicks; i++)
		{
			...
			dc.MoveTo(x = (lpDrawItemStruct->rcItem.left + GRAPH_XMARGIN + GRAPH_XLABELSPACE + GRAPH_TICKSIZE),
				y = lpDrawItemStruct->rcItem.top + GRAPH_YMARGIN + YScale * (MaxY - (v = YMinTick + i * YTicks)));
