	...
	: CDialog(CEditConfig::IDD, pParent), O(o)
{
	...
	m_GS.ClusteringProcessors = 1;
	m_GS.RefreshCycleTime = 50;
	m_GS.ProcessingStartDelayPerFrame = 0.0f;
}
...
void CEditConfig::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EC_PROCESSINGSTARTDELAY, m_GS.ProcessingStartDelayPerFrame);
	DDV_MinMaxFloat(pDX, m_GS.ProcessingStartDelayPerFrame, 0.0f, 100.0f);
...
void CEditConfig::OnSelFilterEq() 
{
	...
	CFileDialog MyDlg(TRUE, "rf", NULL, OFN_FILEMUSTEXIST, "Raw float image (*.rf)|*.rf|All files (*.*)|||", this);
