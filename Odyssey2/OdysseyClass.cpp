...
/*
	pWndDisplay->CreateEx(WS_EX_TOOLWINDOW | WS_EX_CONTROLPARENT, MonitorWndClass = AfxRegisterWndClass(0, ::LoadCursor(AfxGetResourceHandle(), MAKEINTRESOURCE(IDC_ODYSSEY)), m_BackgroundBrush), "Odyssey Display", 
		WS_VISIBLE | WS_POPUP, DisplayChooser.Left, DisplayChooser.Top, DisplayChooser.Width, DisplayChooser.Height, 0, 0, 0);
*/
	pWndDisplay->Init();
...
STDMETHODIMP COdyssey::GetClassName(ConnectionName name)
{
	...
	strncpy(name, "Matrox Odyssey Vision Processor 2", SYSAL_MAX_CONN_NAME_LEN);
	return S_OK;
}
...
STDMETHODIMP COdyssey::VPWaitForProcessingComplete()
{
	...
	FILE *fs = fopen("c:\\acq\\od2.txt", "a+t");
	static int run = 0;
...
/* TEMPORARY!!! */
		if (p.AIS.ExpectedClusters > 0 && p.AIS.AvgClustersTolerance > 0 && p.AIS.ImageCount <= O.MaxOptimizableFramesPerNode)
		{
			...
			if (goodlayers > 0 && fabs((sum /= goodlayers) - p.AIS.ExpectedClusters) > p.AIS.AvgClustersTolerance)
			{
				imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);				
