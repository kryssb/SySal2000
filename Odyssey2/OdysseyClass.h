...
class ATL_NO_VTABLE COdyssey : 
	...
	public CComCoClass<COdyssey, &CLSID_Odyssey>,
	public IVisionProcessor4,
...
BEGIN_COM_MAP(COdyssey)
	...
	COM_INTERFACE_ENTRY(IVisionProcessor4)
...
// IVisionProcessor4
public:
	STDMETHOD(VPWaitForProcessingCompleteAndExtractImages)(BYTE **ppImages);

protected:
	void ProgramProcessing(bool SkipImagePreprocessing = false, bool StoreConv = false, bool SkipInvalidLayers = true);
	ConnectionName Name;
	HANDLE HStatusChangeEvent;

	SySalConnection Slots[NUMBER_OF_SLOTS];

	LPCTSTR MonitorWndClass;
	OdysseyData O;
	bool Initialization;
	...
	void SendProcessingCommands(UINT ActiveProgram, int SequenceIndex, int ImageCount, bool SkipImagePreprocessing = false, bool StoreConv = false, bool SkipInvalidLayers = true);
...
public:
	...
	int VP_Internal_GetMaxOptimizableFrames();
