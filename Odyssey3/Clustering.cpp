...
int COdyssey::GetSegments(BYTE *ScanLine, unsigned Length, Segment *SegSpace, PrecompSegmentBlock *PrecData)
{
	/* 
		REGISTERS USAGE 
...
IntCluster *COdyssey::ChainSegments(Segment *Line1, Segment *Line2, unsigned short SegNum1,
	...
	IntCluster *NextFree)
{
	...
	Start1 = Line1;
	for (SegPointer1 = Start1; SegPointer1 < SegEnd1; SegPointer1++)
	{
		SegPointer1->Left--;
		SegPointer1->Right++;
		}
...
DWORD COdyssey::GetClusters(BYTE *ImageBuffer, PrecompSegmentBlock *PrecompData, Cluster *pSpace, int *pMaxNumber, 
