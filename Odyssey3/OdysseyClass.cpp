...
#include "Objective.h"
#include <process.h>
#undef CRASH_DEBUG_TRACE

#ifdef CRASH_DEBUG_TRACE
#define CRASH_DEBUG_MARK { FILE *__f__ = fopen("c:\\acq\\od3_crash_debug.txt", "a+t"); fprintf(__f__, "%s %d\n", __FUNCTION__, __LINE__); fclose(__f__); }
#define CRASH_DEBUG_MARK_2 { FILE *__f__ = fopen("c:\\acq\\od3_crash_debug_2.txt", "a+t"); fprintf(__f__, "%s %d\n", __FUNCTION__, __LINE__); fclose(__f__); }
#else
#define CRASH_DEBUG_MARK
#define CRASH_DEBUG_MARK_2
#endif
...
COdyssey::COdyssey()
{
	...
	imDevInquire(O.Dev, IM_DEV_FREE_MEM_PROC, &freemem);
	freemem -= 32768 * 1024;
	...
	imsize = O.ImageWidth * O.ImageHeight;
	freemem /= imsize;
	O.MaxFramesPerNode = freemem / 2;
	O.MaxOptimizableFramesPerNode = freemem / 8;
	pOvrSave = (BYTE *)GlobalAlloc(GMEM_FIXED, O.ImageWidth * O.ImageHeight);
	imBufAllocControl(O.ProcThread, &O.GrabCtlBuff);
	imBufPutField(O.ProcThread, O.GrabCtlBuff, IM_CTL_GRAB_MODE, IM_ASYNCHRONOUS);
	...
	for (i = 0; i < O.MaxFramesPerNode; i++)
		for (d = 0; d < MAX_XOP_BUFFERS; d++)
		{
			O.XopSrcBuffs[i][d] = 0;
			O.XopDestBuffs[i][d] = 0;
			}
	...
	imBufAlloc2d(O.ProcThread, O.ImageWidth, O.ImageHeight * O.MaxFramesPerNode, IM_UBYTE, IM_PROC, &O.SeqMainBuff);
	imBufAlloc2d(O.ProcThread, O.ImageWidth, O.ImageHeight * O.MaxFramesPerNode, IM_BINARY, IM_HOST, &O.SeqMainHostBinBuff[0]);
	imBufAlloc2d(O.ProcThread, O.ImageWidth, O.ImageHeight * O.MaxFramesPerNode, IM_BINARY, IM_HOST, &O.SeqMainHostBinBuff[1]);
	for (i = 0; i < O.MaxOptimizableFramesPerNode; i++)
	{
		imBufAlloc2d(O.ProcThread, O.ImageWidth, O.ImageHeight, IM_SHORT, IM_PROC, &O.ConvResBuffs[0][i]);
		imBufAlloc2d(O.ProcThread, O.ImageWidth, O.ImageHeight, IM_SHORT, IM_PROC, &O.ConvResBuffs[1][i]);
		}
	for (i = 0; i < O.MaxFramesPerNode; i++)
	{		
		imBufChild(O.ProcThread, O.SeqMainBuff, 0, i * O.ImageHeight, O.ImageWidth, O.ImageHeight, &O.SeqChildBuffs[i]);
	...
	imBufAlloc1d(O.ProcThread, O.MaxFramesPerNode, IM_DOUBLE, IM_PROC_SHARED, &O.SeqStartTimeBuff);
	...
	imBufAlloc1d(O.ProcThread, O.MaxFramesPerNode, IM_DOUBLE, IM_PROC_SHARED, &O.SeqEndTimeBuff);
	...
	O.SrcGrabBuffs[0] = O.SeqChildBuffs[0];
	O.SrcGrabBuffs[1] = O.SeqChildBuffs[1];
	O.SrcGrabBuffs[2] = O.SeqChildBuffs[2];
...
//	imBufClear(O.ProcThread, O.OverlayBuff, ODYSSEY_TRANSPARENT, 0);
	...
	if (DisplayChooser.ChosenNumber == 0)
	{
		IsDisplaySmall = true;// DisplayChooser.Left = DisplayChooser.Width;
		DisplayChooser.Left = 0;
		DisplayChooser.Top = 0;
		DisplayChooser.Width = O.ImageWidth / 8 + 256;
		DisplayChooser.Height = O.ImageHeight / 8;
		if (DisplayChooser.Height < 512) DisplayChooser.Height = 512;
		}
	else if (DisplayChooser.Width - O.ImageWidth < 256) DisplayChooser.Width += 256;	
	m_BackgroundBrush = CreateSolidBrush(RGB(64, 128, 224));

	volatile HWND hTop = 0;//::GetTopWindow(0);
	...
	wndcls.hIcon = 0;
	wndcls.hCursor = ::LoadCursor(AfxGetResourceHandle(), MAKEINTRESOURCE(IDC_ODYSSEY));
	wndcls.hbrBackground = m_BackgroundBrush;
	wndcls.lpszMenuName = 0;
	wndcls.lpszClassName = MonitorWndClass = "Odyssey Monitor Window";
	/*MonitorWndClass = */RegisterClass(&wndcls);
	...
	pWndDisplay->CreateEx(WS_EX_TOOLWINDOW | WS_EX_CONTROLPARENT, MonitorWndClass, "Odyssey Display", 
		WS_VISIBLE | WS_POPUP, DisplayChooser.Left, DisplayChooser.Top, DisplayChooser.Width, DisplayChooser.Height, hTop, 0, 0);
...
//	imBufFree(O.ProcThread, O.UnderlayBuff);
	...
	for (i = 0; i < O.MaxFramesPerNode; i++)
		for (d = 0; d < MAX_XOP_BUFFERS; d++)
		{
			if (O.XopSrcBuffs[i][d]) imBufFree(O.ProcThread, O.XopSrcBuffs[i][d]);
			O.XopSrcBuffs[i][d] = 0;
			if (O.XopDestBuffs[i][d]) imBufFree(O.ProcThread, O.XopDestBuffs[i][d]);
			O.XopDestBuffs[i][d] = 0;
			}
	for (i = 0; i < O.MaxOptimizableFramesPerNode; i++)
	{
		imBufFree(O.ProcThread, O.ConvResBuffs[0][i]);
		imBufFree(O.ProcThread, O.ConvResBuffs[1][i]);
		}
	for (i = 0; i < O.MaxFramesPerNode; i++)
	{
		imBufFree(O.ProcThread, O.SeqChildBuffs[i]);
		imBufFree(O.ProcThread, O.SeqChildHostBinBuffs[0][i]);
		imBufFree(O.ProcThread, O.SeqChildHostBinBuffs[1][i]);
		}
	imBufFree(O.ProcThread, O.SeqMainBuff);
	...
	imBufFree(O.ProcThread, O.GrabCtlBuff);
	...
	volatile BOOL unreg = UnregisterClass(MonitorWndClass, AfxGetInstanceHandle());
...
*/
	...
	for (i = 0; i < O.MaxFramesPerNode; i++)
		...
		{
			imBufChild(O.ProcThread, O.SeqMainBuff, O.ExtendedOperation.OpDescriptions.Desc[d].Src.left, 
				...
				&O.XopSrcBuffs[i][d]);
			imBufChild(O.ProcThread, O.SeqMainBuff, O.ExtendedOperation.OpDescriptions.Desc[d].Dest.left, 
...
STDMETHODIMP COdyssey::GetClassName(ConnectionName name)
{
	...
	strncpy(name, "Matrox Odyssey Vision Processor 3", SYSAL_MAX_CONN_NAME_LEN);
	return S_OK;
}
...
{  
   CRASH_DEBUG_MARK
	...
	WaitForSingleObject(HRefreshDeviceMutex, INFINITE);
	CommandCount += 1;
	imDigGrab(O.GrabThread, 0, O.Cam, O.FreezeBuff, 1, O.GrabCtlBuff, O.GrabOSB[0]);
...
//	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	...
	imSyncHost(O.SeqStartThread, 0, IM_COMPLETED);
	imSyncHost(O.ProcThread, 0, IM_COMPLETED);
...
//	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	...
	imDigGrab(O.GrabThread, 0, O.Cam, O.SrcGrabBuffs[0], 1, O.GrabCtlBuff, O.GrabOSB[0]);
...
//	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	...
	imDigGrab(O.GrabThread, 0, O.Cam, O.SrcGrabBuffs[hs], 1, O.GrabCtlBuff, O.GrabOSB[h]);
	...
	for (j = 0; j < O.ExtendedOperation.OpDescriptions.Count; j++)
		...
		{
			case XOP_TYPE_COPY:		imBufCopy(O.ProcThread, O.XopSrcBuffs[hso][j], O.XopDestBuffs[hso][j], 0, 0); break;
			case XOP_TYPE_HFLIP:	imIntFlip(O.ProcThread, O.XopSrcBuffs[hso][j], O.XopDestBuffs[hso][j], IM_FLIP_H, IM_DEFAULT, 0); break;
			case XOP_TYPE_VFLIP:	imIntFlip(O.ProcThread, O.XopSrcBuffs[hso][j], O.XopDestBuffs[hso][j], IM_FLIP_V, IM_DEFAULT, 0); break;
			case XOP_ROTATE_90:		imIntFlip(O.ProcThread, O.XopSrcBuffs[hso][j], O.XopDestBuffs[hso][j], IM_ROTATE_90, IM_DEFAULT, 0); break;
			case XOP_ROTATE_180:	imIntFlip(O.ProcThread, O.XopSrcBuffs[hso][j], O.XopDestBuffs[hso][j], IM_ROTATE_180, IM_DEFAULT, 0); break;
			case XOP_ROTATE_270:	imIntFlip(O.ProcThread, O.XopSrcBuffs[hso][j], O.XopDestBuffs[hso][j], IM_ROTATE_270, IM_DEFAULT, 0); break;
			}
	// copy to display...
#ifndef ENABLE_GAIN_COMPENSATION
...
STDMETHODIMP COdyssey::SetLightLevel()
{
	...
	while (minlev < maxlev)
	{
		...
		{
			imDigGrab(O.GrabThread, 0, O.Cam, O.GrabBuff, 1, O.GrabCtlBuff, O.GrabOSB[0]);	
			// display...
			imSyncHost(O.GrabThread, O.GrabOSB[0], IM_COMPLETED);
			}
		LC_Get_LightInfo(&graylev, &satlev, &fdummy, &pidummy);
...
{
	return O.MaxFramesPerNode;
}

int COdyssey::VP_Internal_GetMaxOptimizableFrames()
{
	return O.MaxOptimizableFramesPerNode;
}
...
float COdyssey::VP_Internal_GetFrameRate()
{
	...
	imDigGrabSequence(O.GrabThread, 0, O.Cam, O.SeqMainBuff, TESTINGCYCLES, O.SeqCtlBuff, O.SeqOSB);
...
STDMETHODIMP COdyssey::VPPrepareAsyncProcessing(UINT Program, AsyncImageSequence ImageSeq)
{
	...
	// TODO: Add your implementation code here
	if (Program >= O.ProgramLibrary.Count) return E_INVALIDARG;
	if (ImageSeq.ImageCount > O.MaxFramesPerNode) return E_INVALIDARG;

	int seqindex = (O.SequenceIndex + 1) % 2;
	OdysseyData::t_SequenceProgram &p = O.SequencePrograms[seqindex];
	p.ActiveProgram = Program;

	if (p.Status == VP_PROGRAM_RUNNING || p.Status == VP_PROGRAM_CLUSTERING) return S_FALSE;
...
STDMETHODIMP COdyssey::VPStartAsyncProcessing()
{
	...
	imSyncControl(O.ProcThread, O.ProcOSB, IM_OSB_STATE, IM_WAITING);
	imDigGrabSequence(O.GrabThread, 0, O.Cam, O.SeqMainBuff, O.SequencePrograms[seqindex].AIS.ImageCount, O.SeqCtlBuff, O.SeqOSB);
...
STDMETHODIMP COdyssey::VPWaitForProcessingComplete()
{
	...
	if (!O.SequencePrograms[O.SequenceIndex].ProcessingProgrammed) 
		ProgramProcessing(false, O.SequencePrograms[O.SequenceIndex].AIS.ExpectedClusters > 0 && O.SequencePrograms[O.SequenceIndex].AIS.ImageCount < O.MaxOptimizableFramesPerNode);

	imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);

//	FILE *fs = fopen("c:\\acq\\od3.txt", "a+t");
	static int run = 0;

	run++;

	for (i = 0; i < O.SequencePrograms[O.SequenceIndex].AIS.ImageCount; i++)
	{	O.SequencePrograms[O.SequenceIndex].AIS.pImages[i].TimeStamp = /*1000.0f*/ 1000000.0f * 
			(O.pSeqStartTimeBuff[i] - O.SeqTimeZero[O.SequenceIndex]);
//		fprintf(fs, "%d %d %d\n", run, i, O.SequencePrograms[O.SequenceIndex].AIS.pImages[i].TimeStamp);
		}
...
STDMETHODIMP COdyssey::VPWaitForProcessingCompleteAndExtractImages(BYTE **ppImages)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState())
	CRASH_DEBUG_MARK
	// TODO: Add your implementation code here
	...
	for (i = 0; i < O.SequencePrograms[O.SequenceIndex].AIS.ImageCount; i++)
		O.SequencePrograms[O.SequenceIndex].AIS.pImages[i].TimeStamp = 1000000.0f * 
			(O.pSeqStartTimeBuff[i] - O.SeqTimeZero[O.SequenceIndex]);
	O.ClusteringSequenceIndex = O.SequenceIndex;
/*
...
*/
	...
	for (i = 0; i < O.SequencePrograms[O.SequenceIndex].AIS.ImageCount; i++)
		imBufGet(O.ProcThread, O.SeqChildBuffs[i], ppImages[i]);
...
STDMETHODIMP COdyssey::VPWaitForClusters()
{
	...
	{
/* TEMPORARY!!! */
		int *pmaxclusters = new int[p.AIS.ImageCount];
		for (i = 0; i < p.AIS.ImageCount; i++)
			pmaxclusters[i] = p.AIS.pImages[i].ClusterCount;
/* End TEMPORARY */
		...
		if (O.GeneralSettings.ClusteringProcessors > 1) WaitForMultipleObjects(O.GeneralSettings.ClusteringProcessors - 1, HFinished + 1, TRUE, INFINITE);
/* TEMPORARY!!! */
		if (p.AIS.ExpectedClusters > 0 && p.AIS.AvgClustersTolerance > 0 && p.AIS.ImageCount <= O.MaxOptimizableFramesPerNode)
		{
			int sum, max = 0;
			int ccount, j;
			int testlev = 0, goodlayers = 0;
			for (i = sum = 0; i < p.AIS.ImageCount; i++)
			{
				for (ccount = j = 0; j < p.AIS.pImages[i].ClusterCount; j++)
					if (p.AIS.pImages[i].pClusters[j].Area >= p.AIS.MinClusterArea) ccount++;
				if (ccount/*p.AIS.pImages[i].ClusterCount*/ > p.AIS.MinClusters)
				{
					goodlayers++;
					if (ccount/*p.AIS.pImages[i].ClusterCount*/ > max) 
					{
						testlev = i;					
						max = ccount;//p.AIS.pImages[i].ClusterCount;
						}
					sum += ccount;//p.AIS.pImages[i].ClusterCount;
					}
				}
			if (goodlayers > 0 && fabs((double)(sum /= goodlayers) - p.AIS.ExpectedClusters) > p.AIS.AvgClustersTolerance)
			{
				imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);				
				int ccount_0, ccount_m_1, ccount_p_1;
				float s = O.ProgramLibrary.Programs[p.ActiveProgram].Threshold * 0.1;
				int step = s;
				ccount_0 = max;				

				do
				{
					if (ccount_0 < p.AIS.MinClusters)
					{
						O.ProgramLibrary.Programs[p.ActiveProgram].LastThreshold = O.ProgramLibrary.Programs[p.ActiveProgram].Threshold;
						break;
						}
					float f_0 = (float)ccount_0 / (float)max * (float)sum / (float)p.AIS.ExpectedClusters;
					ccount_m_1 = ccount_p_1 = pmaxclusters[testlev];

					if (O.FilterEqBuffs[p.ActiveProgram]) 
						imBinConvert(O.ProcThread, O.ConvResBuffs[O.ClusteringSequenceIndex][testlev], O.BinBuff, IM_LESS, O.ProgramLibrary.Programs[p.ActiveProgram].LastThreshold - O.ProgramLibrary.Programs[p.ActiveProgram].Threshold - step, 0, 0);
					else imBinConvert(O.ProcThread, O.ConvResBuffs[O.ClusteringSequenceIndex][testlev], O.BinBuff, IM_LESS, O.ProgramLibrary.Programs[p.ActiveProgram].LastThreshold - step, 0, 0);
					imBufCopy(O.ProcThread, O.BinBuff, O.SeqChildHostBinBuffs[O.ClusteringSequenceIndex][testlev], 0, O.ProcOSB);
					imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);
				
					if (GetClusters(O.pSeqChildHostBinBuffs[O.ClusteringSequenceIndex][testlev], PrecompData, p.AIS.pImages[testlev].pClusters, &ccount_m_1, O.ImageWidth, O.ImageHeight, 
						O.ProgramLibrary.Programs[p.ActiveProgram].OffX, O.ProgramLibrary.Programs[p.ActiveProgram].OffY, 
						O.ProgramLibrary.Programs[p.ActiveProgram].WinWidth, O.ProgramLibrary.Programs[p.ActiveProgram].WinHeight, 
						O.SegSpace, O.SegSpace + O.ImageWidth) != S_OK)
					{
						O.ProgramLibrary.Programs[p.ActiveProgram].LastThreshold = O.ProgramLibrary.Programs[p.ActiveProgram].Threshold;
						break;
						}
					for (ccount = j = 0; j < ccount_m_1; j++)
						if (p.AIS.pImages[testlev].pClusters[j].Area >= p.AIS.MinClusterArea) ccount++;
					ccount_m_1 = ccount;
					...
					if (GetClusters(O.pSeqChildHostBinBuffs[O.ClusteringSequenceIndex][testlev], PrecompData, p.AIS.pImages[testlev].pClusters, &ccount_p_1, O.ImageWidth, O.ImageHeight, 
						O.ProgramLibrary.Programs[p.ActiveProgram].OffX, O.ProgramLibrary.Programs[p.ActiveProgram].OffY, 
						O.ProgramLibrary.Programs[p.ActiveProgram].WinWidth, O.ProgramLibrary.Programs[p.ActiveProgram].WinHeight, 
						O.SegSpace, O.SegSpace + O.ImageWidth) != S_OK)
					{
					...
					for (ccount = j = 0; j < ccount_p_1; j++)
						if (p.AIS.pImages[testlev].pClusters[j].Area >= p.AIS.MinClusterArea) ccount++;
					ccount_p_1 = ccount;
				
					float f_m_1 = (float)ccount_m_1 / (float)ccount_0 * f_0;
					float f_p_1 = (float)ccount_p_1 / (float)ccount_0 * f_0;
					float a = (f_m_1 + f_p_1 - 2.0 * f_0) / (2.0 * s * s);
					float b = (f_p_1 - f_m_1) / (2.0 * s);
					float c = f_0;
					float d = 4.0 * a + b * b - 4.0 * a * c;
					float x = (d > 0 && a > 0) ? (-(b + sqrt(d)) / (2.0 * a)) : ((1 - c) / b) ;
					O.ProgramLibrary.Programs[p.ActiveProgram].LastThreshold += (int)x;

					if (O.FilterEqBuffs[p.ActiveProgram]) 
						imBinConvert(O.ProcThread, O.ConvResBuffs[O.ClusteringSequenceIndex][testlev], O.BinBuff, IM_LESS, O.ProgramLibrary.Programs[p.ActiveProgram].LastThreshold - O.ProgramLibrary.Programs[p.ActiveProgram].Threshold, 0, 0);
					else imBinConvert(O.ProcThread, O.ConvResBuffs[O.ClusteringSequenceIndex][testlev], O.BinBuff, IM_LESS, O.ProgramLibrary.Programs[p.ActiveProgram].LastThreshold, 0, 0);
					imBufCopy(O.ProcThread, O.BinBuff, O.SeqChildHostBinBuffs[O.ClusteringSequenceIndex][testlev], 0, O.ProcOSB);
					imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);

					ccount_0 = pmaxclusters[testlev];
					if (GetClusters(O.pSeqChildHostBinBuffs[O.ClusteringSequenceIndex][testlev], PrecompData, p.AIS.pImages[testlev].pClusters, &ccount_0, O.ImageWidth, O.ImageHeight, 
						O.ProgramLibrary.Programs[p.ActiveProgram].OffX, O.ProgramLibrary.Programs[p.ActiveProgram].OffY, 
						O.ProgramLibrary.Programs[p.ActiveProgram].WinWidth, O.ProgramLibrary.Programs[p.ActiveProgram].WinHeight, 
						O.SegSpace, O.SegSpace + O.ImageWidth) != S_OK)
					{
					...
					for (ccount = j = 0; j < ccount_0; j++)
						if (p.AIS.pImages[testlev].pClusters[j].Area >= p.AIS.MinClusterArea) ccount++;
					ccount_0 = ccount;

				}
				while (fabs((float)ccount_0 / (float)max * (float)sum - p.AIS.ExpectedClusters) > p.AIS.AvgClustersTolerance);

				for (i = 0; i < p.AIS.ImageCount; i++)
					p.AIS.pImages[i].ClusterCount = pmaxclusters[i];
				for (i = 0; i < p.AIS.ImageCount; i++)
				{
					if (O.FilterEqBuffs[p.ActiveProgram]) 
						imBinConvert(O.ProcThread, O.ConvResBuffs[O.ClusteringSequenceIndex][i], O.BinBuff, IM_LESS, O.ProgramLibrary.Programs[p.ActiveProgram].LastThreshold - O.ProgramLibrary.Programs[p.ActiveProgram].Threshold, 0, 0);
					else imBinConvert(O.ProcThread, O.ConvResBuffs[O.ClusteringSequenceIndex][i], O.BinBuff, IM_LESS, O.ProgramLibrary.Programs[p.ActiveProgram].LastThreshold, 0, 0);
					imBufCopy(O.ProcThread, O.BinBuff, O.SeqChildHostBinBuffs[O.ClusteringSequenceIndex][i], 0, 0);
					}
				imThrNop(O.ProcThread, O.ProcOSB);
				imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);

				ProgramProcessing(true, false);
...
void COdyssey::ProgramProcessing(bool SkipImagePreprocessing, bool StoreConv, bool SkipInvalidLayers)
{
	int i, j;
	CRASH_DEBUG_MARK
	OdysseyData::t_SequenceProgram &p = O.SequencePrograms[O.SequenceIndex];

	if (!SkipImagePreprocessing)
	{
		imSyncGrabSequence(O.SeqStartThread, O.SeqOSB, 0, IM_SEQ_WAIT);
		imIntHistogram(O.SeqStartThread, O.SeqChildBuffs[0], O.HistoBuff, IM_DEFAULT, O.GrabOSB[0]);
		...
		for (i = 0; i < 256; i++) 
			O.pLUT[i] = (corr * i <= 255) ? (corr * i) : 255;
	...
	SendProcessingCommands(p.ActiveProgram, O.SequenceIndex, p.AIS.ImageCount, SkipImagePreprocessing, StoreConv, SkipInvalidLayers);
	imThrNop(O.ProcThread, O.ProcOSB);
	p.ProcessingProgrammed = true;
	CRASH_DEBUG_MARK
}

void COdyssey::SendProcessingCommands(UINT ActiveProgram, int SequenceIndex, int ImageCount, bool SkipImagePreprocessing, bool StoreConv, bool SkipInvalidLayers)
{
	CRASH_DEBUG_MARK
	int i, j;	
	for (i = 0; i < ImageCount; i++)
	{
		if (SkipInvalidLayers && O.SequencePrograms[SequenceIndex].AIS.pImages[i].ClusterCount == 0) continue; // save time
#ifndef REENABLE_STABILIZATION
		imSyncGrabSequence(O.ProcThread, O.SeqOSB, i, IM_SEQ_WAIT_THREAD);
#endif
		if (!SkipImagePreprocessing)
		{
			if (O.FlatFieldBuffs[ActiveProgram]) imIntDyadic(O.ProcThread, O.SeqChildBuffs[i], O.FlatFieldBuffs[ActiveProgram], O.SeqChildBuffs[i], IM_SUB, 0);
			imIntLutMap(O.ProcThread, O.SeqChildBuffs[i], O.SeqChildBuffs[i], O.LUTBuff, 0);
			for (j = 0; j < O.ExtendedOperation.OpDescriptions.Count; j++)
				switch (O.ExtendedOperation.OpDescriptions.Desc[j].Type)
				{
					case XOP_TYPE_COPY:		imBufCopy(O.ProcThread, O.XopSrcBuffs[i][j], O.XopDestBuffs[i][j], 0, 0); break;
					case XOP_TYPE_HFLIP:	imIntFlip(O.ProcThread, O.XopSrcBuffs[i][j], O.XopDestBuffs[i][j], IM_FLIP_H, IM_DEFAULT, 0); break;
					case XOP_TYPE_VFLIP:	imIntFlip(O.ProcThread, O.XopSrcBuffs[i][j], O.XopDestBuffs[i][j], IM_FLIP_V, IM_DEFAULT, 0); break;
					case XOP_ROTATE_90:		imIntFlip(O.ProcThread, O.XopSrcBuffs[i][j], O.XopDestBuffs[i][j], IM_ROTATE_90, IM_DEFAULT, 0); break;
					case XOP_ROTATE_180:	imIntFlip(O.ProcThread, O.XopSrcBuffs[i][j], O.XopDestBuffs[i][j], IM_ROTATE_180, IM_DEFAULT, 0); break;
					case XOP_ROTATE_270:	imIntFlip(O.ProcThread, O.XopSrcBuffs[i][j], O.XopDestBuffs[i][j], IM_ROTATE_270, IM_DEFAULT, 0); break;
					}
			// copy to display...		
			}

/*
		if (O.FlatFieldBuffs[ActiveProgram] && !SkipImagePreprocessing) imIntDyadic(O.ProcThread, O.SeqChildBuffs[SequenceIndex][i], O.FlatFieldBuffs[ActiveProgram], O.SeqChildBuffs[SequenceIndex][i], IM_SUB, 0);
  ...
  */
#ifndef ENABLE_GAIN_COMPENSATION
		long convbuff = StoreConv ? O.ConvResBuffs[SequenceIndex][i] : O.ConvBuff;
		imIntConvolve(O.ProcThread, O.SeqChildBuffs[i], convbuff, O.KernelBuffs[ActiveProgram], 0, 0);
		if (O.FilterEqBuffs[ActiveProgram]) 
		{
			imIntDyadic(O.ProcThread, convbuff, O.FilterEqBuffs[ActiveProgram], convbuff, IM_SUB, 0);
			imBinConvert(O.ProcThread, convbuff, O.BinBuff, IM_LESS, (StoreConv ||  O.ForcedThreshold) ? (O.ProgramLibrary.Programs[ActiveProgram].LastThreshold - O.ProgramLibrary.Programs[ActiveProgram].Threshold) : 0, 0, 0);
			}
/* TEMPORARY!!! */
		else imBinConvert(O.ProcThread, convbuff, O.BinBuff, IM_LESS, (StoreConv || O.ForcedThreshold) ? O.ProgramLibrary.Programs[ActiveProgram].LastThreshold : O.ProgramLibrary.Programs[ActiveProgram].Threshold, 0, 0);
/* End TEMPORARY */
...
UINT COdyssey::RefreshThreadHook(void *owner)
{
	...
	if (This->IsDisplaySmall && This->O.GeneralSettings.RefreshCycleTime < 1000) nexttime = GetTickCount() + 1000;
	else nexttime = GetTickCount() + This->O.GeneralSettings.RefreshCycleTime;
	SetEvent(This->O.HRefreshStopped);
	while (wait != WAIT_OBJECT_0 && WaitForMultipleObjects(2, HFlux, FALSE, INFINITE) != WAIT_OBJECT_0)
	{
		ResetEvent(This->O.HRefreshStopped);
		while ((wait = WaitForMultipleObjects(2, HFlux, FALSE, 0)) != WAIT_OBJECT_0)
			if (wait == WAIT_TIMEOUT)
			{
				...
				if (GetTickCount() > nexttime)
				{
					...
					imDigGrab(This->O.GrabThread, 0, This->O.Cam, This->O.GrabBuff, 1, This->O.GrabCtlBuff, This->O.GrabOSB[0]);
...
void COdyssey::VP_Internal_MakeFlatFieldImage(BYTE *pImg, int imgcount)
{
	...
	imSyncControl(O.GrabThread, O.SeqOSB, IM_OSB_STATE, IM_WAITING);
	imDigGrabSequence(O.GrabThread, 0, O.Cam, O.SeqMainBuff, imgcount, O.SeqCtlBuff, O.SeqOSB);
	imSyncHost(O.GrabThread, O.SeqOSB, IM_COMPLETED);
	...
	{
		imBufCopy(O.ProcThread, O.SeqChildBuffs[i], O.FreezeBuff, 0, O.ProcOSB);
		imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);
		for (j = 0; j < imgsize; j++)
...
#ifndef ENABLE_GAIN_COMPENSATION
	__int64 avgsum = 0;
	__int64 varsum, noisesum;
	for (j = 0; j < imgsize; j++) avgsum += pAvg[j];
	avgsum /= (imgsize * imgcount);
	for (j = 0; j < imgsize; j++)
		pImg[j] = (pAvg[j] / imgcount - avgsum);
	avgsum = varsum = 0;
	for (j = 0; j < imgsize; j++) avgsum += (char)pImg[j];
	avgsum /= imgsize;
	for (j = 0; j < imgsize; j++)
		varsum += (char)pImg[j] * (char)pImg[j];
	float spacenoise = sqrt((float)varsum / (imgsize - 1));
	noisesum = 0;
	for (i = 0; i < imgcount; i++)
	{
		imBufCopy(O.ProcThread, O.SeqChildBuffs[i], O.FreezeBuff, 0, O.ProcOSB);
		imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);
		for (j = 0; j < imgsize; j++)
			noisesum += ((int)(unsigned)(unsigned char)O.pFreezeBuff[j] - pAvg[j] / imgcount) * ((int)(unsigned)(unsigned char)O.pFreezeBuff[j] - pAvg[j] / imgcount);
		}
	float timenoise = sqrt((float)noisesum / ((imgsize - 1) * imgcount));
	char resstr[512];
	sprintf(resstr, "Spatial inhomogeneity: %.2f\r\nFluctuation in time: %.2f", spacenoise, timenoise);
	MessageBox(0, resstr, "Results", MB_OK);
#else
	for (j = 0; j < imgsize; j++)
...
void COdyssey::FG_Internal_LoadGrabBuffer(DWORD Context, int bank, int number)
{
	...
	pO->CommandCount += 3;
	imIntMonadic(O.ProcThread, O.SeqChildBuffs[number], ODYSSEY_GRABMIN, O.DisplayHostBuff, IM_MAX, 0);
...
bool COdyssey::VP_Internal_AsyncAcquireWithForcedCutOff(unsigned program, int cutoff, Cluster *pSpace, int *pMaxClusters)
{
	...
	ASI.MinClusters = 0;
// TEMPORARY!!!
	int thresholdbackup = O.ProgramLibrary.Programs[program].LastThreshold;
	O.ProgramLibrary.Programs[program].LastThreshold = cutoff;
// End TEMPORARY!!!
...
// TEMPORARY!!!
	O.ProgramLibrary.Programs[program].LastThreshold = thresholdbackup;
// End TEMPORARY!!!
...
double COdyssey::VP_Internal_MeasureProcessingTime(UINT program)
{
	...
	SendProcessingCommands(program, 0, PROCESS_TIME_CYCLES, false, false, false);
	imThrNop(O.ProcThread, O.ProcOSB);
	imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);
...
void COdyssey::VP_Internal_OptimizeLocalThreshold(int program, int xsamples, int ysamples, int w, int h, float minmult, float maxmult, int minarea, int steps, int *pcounts)
{
	...
	imDigGrabSequence(O.GrabThread, 0, O.Cam, O.SeqMainBuff, 1, O.SeqCtlBuff, O.SeqOSB);
	imSyncHost(O.SeqStartThread, O.SeqOSB, IM_STARTED);
	ProgramProcessing(false, false, false);	
	imThrNop(O.ProcThread, O.ProcOSB);
	imSyncHost(O.ProcThread, O.ProcOSB, IM_COMPLETED);
	int MaxClusters;
