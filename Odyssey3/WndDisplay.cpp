...
CWndDisplay::CWndDisplay(OdysseyData *pO, bool issmall, int width, int height, int imagewidth, int imageheight, DWORD context, void (*psetlive)(DWORD), void (*psetstill)(DWORD), void (*ploadgrabbuffer)(DWORD,int,int), void (*pclear)(DWORD), void (*pdrawcircles)(DWORD))
...
{
	IsSmall = issmall;
	GrabBuffers = pO->MaxFramesPerNode;
	pCursorX = &pO->CursorX;
	pCursorY = &pO->CursorY;
	pCursorAlignX = &pO->CursorAlignX;
	pCursorAlignY = &pO->CursorAlignY;
	pCursorVisible = &pO->CursorVisible;
	Info.Hdr.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	Info.Hdr.bmiHeader.biWidth = imagewidth / (issmall ? 8 : 1);
	Info.Hdr.bmiHeader.biHeight = -imageheight / (issmall ? 8 : 1);
	...
	m_Data = (BYTE *)GlobalAlloc(GMEM_FIXED, imagewidth * imageheight / (issmall ? 64 : 1));
	m_LiveImage = true;
	m_Locked = false;
...
void CWndDisplay::SetImage(BYTE *pData)
{
	...
	if (pData)
	{
		if (IsSmall)
		{
			...
			unsigned g;
			BYTE *p;
			for (iy = 0; iy < (-Info.Hdr.bmiHeader.biHeight); iy++)
				for (ix = 0; ix < Info.Hdr.bmiHeader.biWidth; ix++)
				{
					g = 0;
					for (iiy = 0; iiy < 8; iiy++)
					{	
						p = pData + ((iy * 8 + iiy) * Info.Hdr.bmiHeader.biWidth + ix) * 8;
						g += (unsigned int)(unsigned char)p[0] + (unsigned int)(unsigned char)p[1] + (unsigned int)(unsigned char)p[2] + (unsigned int)(unsigned char)p[3] + 
							(unsigned int)(unsigned char)p[4] + (unsigned int)(unsigned char)p[5] + (unsigned int)(unsigned char)p[6] + (unsigned int)(unsigned char)p[7];
						}
					g /= 64;
					m_Data[iy * Info.Hdr.bmiHeader.biWidth + ix] = g;
					}
			}
		else memcpy(m_Data, pData, Info.Hdr.bmiHeader.biWidth * (-Info.Hdr.bmiHeader.biHeight));
...
void CWndDisplay::LiveImage(BYTE *pData)
{
	CClientDC dc(this);
	RECT EnlargeRect;
	EnlargeRect.left = Info.Hdr.bmiHeader.biWidth + 32;
	EnlargeRect.top = 512;
	EnlargeRect.bottom = EnlargeRect.top + 136;
	EnlargeRect.right = EnlargeRect.left + 136;
	CBrush EnlargeBrush(RGB(128, 128, 192));
	CPen EnlargePen(PS_SOLID, 1, RGB(224, 224, 255));
	POINT EnlargePerimeter[5] =
	{ 
		{ EnlargeRect.left - 1, EnlargeRect.top - 1 },
		{ EnlargeRect.right, EnlargeRect.top - 1 },
		{ EnlargeRect.right, EnlargeRect.bottom },
		{ EnlargeRect.left - 1, EnlargeRect.bottom },
		{ EnlargeRect.left - 1, EnlargeRect.top - 1 }
	};
	...
	if (pData)
	{
		if (IsSmall)
		{
			...
			SetDIBitsToDevice(dc.m_hDC, 0, 0, Info.Hdr.bmiHeader.biWidth, -Info.Hdr.bmiHeader.biHeight, 0, 0, 0, -Info.Hdr.bmiHeader.biHeight, m_Data, &Info.Hdr, DIB_RGB_COLORS);
			if (*pCursorVisible) StretchDIBits(dc.m_hDC, EnlargeRect.left, EnlargeRect.top, EnlargeRect.right - EnlargeRect.left, EnlargeRect.bottom - EnlargeRect.top, *pCursorX - 8, -Info.Hdr.bmiHeader.biHeight - (*pCursorY + 9), 17, 17, pData, &Info.Hdr, DIB_RGB_COLORS, SRCCOPY);
			else dc.FillRect(&EnlargeRect, &EnlargeBrush);
			dc.SelectObject(&EnlargePen);
			dc.Polyline(EnlargePerimeter, 5);
			}
		else 
		{
			SetDIBitsToDevice(dc.m_hDC, 0, 0, Info.Hdr.bmiHeader.biWidth, -Info.Hdr.bmiHeader.biHeight, 0, 0, 0, -Info.Hdr.bmiHeader.biHeight, pData, &Info.Hdr, DIB_RGB_COLORS);
			if (*pCursorVisible) StretchDIBits(dc.m_hDC, EnlargeRect.left, EnlargeRect.top, EnlargeRect.right - EnlargeRect.left, EnlargeRect.bottom - EnlargeRect.top, *pCursorX - 8, -Info.Hdr.bmiHeader.biHeight - (*pCursorY + 9), 17, 17, pData, &Info.Hdr, DIB_RGB_COLORS, SRCCOPY);
			else dc.FillRect(&EnlargeRect, &EnlargeBrush);
			dc.SelectObject(&EnlargePen);
			dc.Polyline(EnlargePerimeter, 5);
...
void CWndDisplay::Init()
{
	...
	SaveImage.CreateEx(0, "Button", "Save &Image", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_SAVE);
	//SaveImage.Create("Save &Image", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_SAVE);
	SaveImage.RedrawWindow();
	rect.top = 150;
	rect.bottom = rect.top + BUTTON_HEIGHT;
	SetLive.CreateEx(0, "Button", "&Live", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_LIVE);
	//SetLive.Create("&Live", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_LIVE);
	SetLive.RedrawWindow();
	rect.top = 180;
	rect.bottom = rect.top + BUTTON_HEIGHT;
	SetStill.CreateEx(0, "Button", "&Still", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_STILL);
	//SetStill.Create("&Still", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_STILL);	
	SetStill.RedrawWindow();
	rect.top = 210;
	rect.bottom = rect.top + BUTTON_HEIGHT;
	LoadGrabBuffer.CreateEx(0, "Button", "Load &Grab Buffer", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_LOADGRABBUFF);
	//LoadGrabBuffer.Create("Load &Grab Buffer", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_LOADGRABBUFF);
	LoadGrabBuffer.RedrawWindow();
	rect.top = 250;
	rect.bottom = rect.top + BUTTON_HEIGHT;
	ClearDisplay.CreateEx(0, "Button", "&Clear", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_CLEAR);
	//ClearDisplay.Create("&Clear", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_CLEAR);
	ClearDisplay.RedrawWindow();
	rect.top = 280;
	rect.bottom = rect.top + BUTTON_HEIGHT;
	DrawCircles.CreateEx(0, "Button", "&Draw Circles", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_FLAT | BS_OWNERDRAW, rect, this, IDC_WD_DRAWCIRCLES);
...
void CWndDisplay::SetCommandCount(int cmdcount)
{
	CClientDC dc(this);
	dc.SelectObject(&TextFont);
	dc.SetBkColor(RGB(64, 128, 224));
	...
	dc.SetTextAlign(TA_TOP | TA_LEFT);
	char temp[64];
	wsprintf(temp, "Cmd: %d       ", cmdcount);
	dc.TextOut(Info.Hdr.bmiHeader.biWidth + 24, -Info.Hdr.bmiHeader.biHeight - 24, temp, strlen(temp));
	}


void CWndDisplay::OnClose() 
{
	// TODO: Add your message handler code here and/or call default
	OnSetStill();
	CWnd::OnClose();
