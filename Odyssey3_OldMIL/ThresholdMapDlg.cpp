...
BEGIN_MESSAGE_MAP(CThresholdMapDlg, CDialog)
	...
	ON_WM_DRAWITEM()
	ON_EN_KILLFOCUS(IDC_TQ_XSAMPLINGFACTOR, OnKillFocusXSamplingFactor)
	ON_EN_KILLFOCUS(IDC_TQ_YSAMPLINGFACTOR, OnKillFocusYSamplingFactor)
	ON_EN_KILLFOCUS(IDC_TQ_XWAVES, OnKillFocusXWaves)
	ON_EN_KILLFOCUS(IDC_TQ_YWAVES, OnKillFocusYWaves)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CThresholdMapDlg::OnStart() 
{
	...
	UpdateData();
	GetDlgItem(IDC_TQ_START)->EnableWindow(FALSE);
	GetDlgItem(IDOK)->EnableWindow(FALSE);
	GetDlgItem(IDCANCEL)->EnableWindow(FALSE);
	pScanThread = AfxBeginThread((AFX_THREADPROC)ScanHook, this, 0, CREATE_SUSPENDED, NULL);
	pScanThread->m_bAutoDelete = TRUE;
	pScanThread->ResumeThread();
}

void CThresholdMapDlg::DrawGrid(int w, int h)
{
	UpdateData();
	int code;
	...
	{
		cy = Height * (iy + 0.5) / YSamples - h / 2;
		for (ix = 0; ix < XSamples; ix++)
		{
			cx = Width * (ix + 0.5) / XSamples - w / 2;
			O.Rect(cx, cy, cx + w, cy + w, code);
			}
		}
}
...
{
	// TODO: Add your control notification handler code here
	if (UpdateData()) DrawGrid(m_Width, m_Height);
}
...
BOOL CThresholdMapDlg::OnInitDialog() 
{
	...
	DrawGrid(m_Width, m_Height);
	return TRUE;  // return TRUE unless you set the focus to a control
	              // EXCEPTION: OCX Property Pages should return FALSE
}

void CThresholdMapDlg::ScanHook(void *powner)
{
	((CThresholdMapDlg *)powner)->Scan();
	}
...
void CThresholdMapDlg::Scan()
{
	...
	GetDlgItem(IDC_TQ_START)->EnableWindow(TRUE);
...
*/
				sf = (float)(maxclusters - pCounts[(i * YSamples + iy) * XSamples + ix]) / 
					(float)(pCounts[((i - 1) * YSamples + iy) * XSamples + ix] - pCounts[(i * YSamples + iy) * XSamples + ix]);
				pSamples[iy * XSamples + ix] = (i - sf) * multstep + m_MinMultiplier;
				}
			}
...
void CThresholdMapDlg::OnKillFocusXSamplingFactor() 
{
	// TODO: Add your control notification handler code here
	if (UpdateData()) DrawGrid(m_Width, m_Height);
}

void CThresholdMapDlg::OnKillFocusYSamplingFactor() 
{
	// TODO: Add your control notification handler code here
	if (UpdateData()) DrawGrid(m_Width, m_Height);
}

void CThresholdMapDlg::OnKillFocusXWaves() 
{
	// TODO: Add your control notification handler code here
	if (UpdateData()) DrawGrid(m_Width, m_Height);
}

void CThresholdMapDlg::OnKillFocusYWaves() 
{
	// TODO: Add your control notification handler code here
	if (UpdateData()) DrawGrid(m_Width, m_Height);	
}
