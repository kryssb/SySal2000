...
COdyssey::COdyssey()
{
	...
	for (i = 0; i < MAX_OPTIMIZABLE_FRAMES_PER_NODE; i++)
	{
...
STDMETHODIMP COdyssey::VPWaitForProcessingComplete()
{
	...
	if (!O.SequencePrograms[O.SequenceIndex].ProcessingProgrammed) 
		ProgramProcessing(false, O.SequencePrograms[O.SequenceIndex].AIS.ExpectedClusters > 0 && O.SequencePrograms[O.SequenceIndex].AIS.ImageCount < MAX_OPTIMIZABLE_FRAMES_PER_NODE);
...
/* End TEMPORARY */
		...
		if (O.GeneralSettings.ClusteringProcessors > 1) WaitForMultipleObjects(O.GeneralSettings.ClusteringProcessors - 1, HFinished + 1, TRUE, INFINITE);
/* TEMPORARY!!! */
		if (p.AIS.ExpectedClusters > 0 && p.AIS.AvgClustersTolerance > 0 && p.AIS.ImageCount <= MAX_OPTIMIZABLE_FRAMES_PER_NODE)
		{
			int sum, max = 0;
			int ccount, j;
			...
			if (goodlayers > 0 && fabs((sum /= goodlayers) - p.AIS.ExpectedClusters) > p.AIS.AvgClustersTolerance)
			{
				...
				do
				{
					...
					GetClusters(O.pSeqChildHostBinBuffs[O.ClusteringSequenceIndex][testlev], PrecompData, p.AIS.pImages[testlev].pClusters, &ccount_m_1, O.ImageWidth, O.ImageHeight, 
						...
						O.SegSpace, O.SegSpace + O.ImageWidth);
					...
					GetClusters(O.pSeqChildHostBinBuffs[O.ClusteringSequenceIndex][testlev], PrecompData, p.AIS.pImages[testlev].pClusters, &ccount_p_1, O.ImageWidth, O.ImageHeight, 
						O.ProgramLibrary.Programs[p.ActiveProgram].OffX, O.ProgramLibrary.Programs[p.ActiveProgram].OffY, 
						O.ProgramLibrary.Programs[p.ActiveProgram].WinWidth, O.ProgramLibrary.Programs[p.ActiveProgram].WinHeight, 
						O.SegSpace, O.SegSpace + O.ImageWidth);
					...
					GetClusters(O.pSeqChildHostBinBuffs[O.ClusteringSequenceIndex][testlev], PrecompData, p.AIS.pImages[testlev].pClusters, &ccount_0, O.ImageWidth, O.ImageHeight, 
						O.ProgramLibrary.Programs[p.ActiveProgram].OffX, O.ProgramLibrary.Programs[p.ActiveProgram].OffY, 
						O.ProgramLibrary.Programs[p.ActiveProgram].WinWidth, O.ProgramLibrary.Programs[p.ActiveProgram].WinHeight, 
						O.SegSpace, O.SegSpace + O.ImageWidth);						
...
void COdyssey::ProgramProcessing(bool SkipImagePreprocessing, bool StoreConv)
{
	int i, j;
	{
		FILE *f = fopen("c:\\t.txt", "at");
		fprintf(f, "ProgramProcessing %d %d\n", (int)SkipImagePreprocessing, (int)StoreConv);
		fclose(f);
		}
	...
	SendProcessingCommands(p.ActiveProgram, O.SequenceIndex, p.AIS.ImageCount, SkipImagePreprocessing, StoreConv);
	imThrNop(O.ProcThread, O.ProcOSB);
	p.ProcessingProgrammed = true;
}

void COdyssey::SendProcessingCommands(UINT ActiveProgram, int SequenceIndex, int ImageCount, bool SkipImagePreprocessing, bool StoreConv)
{
	int i, j;	
...
/*
  ...
  */
		...
		imIntConvolve(O.ProcThread, O.SeqChildBuffs[SequenceIndex][i], convbuff, O.KernelBuffs[ActiveProgram], 0, 0);
		if (O.FilterEqBuffs[ActiveProgram]) 
		{
			imIntDyadic(O.ProcThread, convbuff, O.FilterEqBuffs[ActiveProgram], convbuff, IM_SUB, 0);
			imBinConvert(O.ProcThread, convbuff, O.BinBuff, IM_LESS, O.ProgramLibrary.Programs[ActiveProgram].LastThreshold - O.ProgramLibrary.Programs[ActiveProgram].Threshold, 0, 0);
			}
/* TEMPORARY!!! */
		else imBinConvert(O.ProcThread, convbuff, O.BinBuff, IM_LESS, O.ProgramLibrary.Programs[ActiveProgram].LastThreshold, 0, 0);
/* End TEMPORARY */
