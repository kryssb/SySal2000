...
BEGIN_MESSAGE_MAP(CMonitor, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_POW_Z, OnButtonPowZ)
...
void CMonitor::OnReset() //Here we write all 0 in ports P4 and P5
{
...
void CMonitor::OnButtonPowZ()//simply write 1 to P1.0  
{
	// TODO: Add your control notification handler code here
	WaitForSingleObject(*pHMonitorMutex, INFINITE);
...
//	  printf("res=%d rep[0]=%d  rep[1]=%d \n",res,rep[0],rep[1]);
	  if(res==0) continue;
	  if(rep[0]=='D' && (rep[1] & 0x40) && !(rep[1] & 0x20)) Reached=true;
	}
	if(tme>=timeout) return -tme;
	else return tme;
...
//	printf("GoOnVAxis(%d, %d) called.\n",position,timeout);
	...
	if(port.Write(str,1)!=1) return -tme;
	  ...
	  if(res==0) continue;
//	  printf("res=%d rep[0]=%d  rep[1]=%d \n",res,rep[0],rep[2]);
	  if(rep[0]=='D' && (rep[2] & 0x40) && !(rep[2] & 0x20)) Reached=true;
	}
	if(tme>=timeout) return -tme;
	else return tme;
...
//	WaitForSingleObject(*pHMonitorMutex, INFINITE);
    ...
    if(MoveEmulsion(0,idest)!=1) {  OnReset(); return;};
//	ReleaseMutex(*pHMonitorMutex);	
...
//	WaitForSingleObject(*pHMonitorMutex, INFINITE);
    ...
    if(MoveEmulsion(isrc,0)!=1) { OnReset(); 	return;};;
...
//         res=ArmVacuumControl(true,pC->VacuumTimeout); //Vacuum ON
	...
	res=ArmVacuumControl(true,pC->VacuumTimeout);
	if(res<0 && isrc==0) {MessageBox("ArmVacuum is not reached in 2 seconds!", "Plate Changer Warning", MB_ICONEXCLAMATION|MB_OK);}
...
//	WaitForSingleObject(*pHMonitorMutex, INFINITE);
 	...
 	if(MoveEmulsion(isrc,idest)!=1) { OnReset(); return;};
//	ReleaseMutex(*pHMonitorMutex);
}
...
//       if(pSt) pSt->WaitForAsyncMotionComplete((DWORD)MyShouldStopCallback, (DWORD)this);
	...
	//	WaitForSingleObject(*pHMonitorMutex, INFINITE);
		if(MoveEmulsion(0,Mto)!=1) { OnReset(); return -1;};
...
//		WaitForSingleObject(*pHMonitorMutex, INFINITE);
		if(MoveEmulsion(Mfrom,Mto)!=1) { OnReset(); return -1;};
//		ReleaseMutex(*pHMonitorMutex);
	}
//	ReleaseMutex(*pHMonitorMutex);
...
*/
//	WaitForSingleObject(*pHMonitorMutex, INFINITE);
	if(MoveEmulsion(Mfrom,0)!=1) {  OnReset(); return -1;};
//	ReleaseMutex(*pHMonitorMutex);
...
//	WaitForSingleObject(*pHMonitorMutex, INFINITE);
	...
	if(MoveEmulsion(0,isrc)!=1) {	SetStatus("Unload Plate is failed. "); OnReset();  return -1;};
...
{
OnHomestage();
if(!OpenComPort(pC->ComPort)) AfxMessageBox("Can't open serial port!");
OnInitialize();	
}
