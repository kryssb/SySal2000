...
CZSpeed::CZSpeed(HANDLE *phmonitormutex, int *pspeedmult, float *pmanspeed, ConfigData *pc, void *pctx, void (*presetlamptimeout)(void *, bool)) 
	: pHMonitorMutex(phmonitormutex), pSpeedMult(pspeedmult), pManSpeed(pmanspeed), pC(pc), pContext(pctx), pResetLampTimeout(presetlamptimeout)
{
}
...
void CZSpeed::Move(float Speed)
{
    ...
    *pManSpeed = Speed;
    ...
    {
		flex_set_op_mode(pC->BoardId, 3, NIMC_VELOCITY);
