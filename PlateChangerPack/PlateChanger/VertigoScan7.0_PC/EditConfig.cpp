...
void CEditConfig::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_EC_SAVEGRAINS, C.SaveGrains);
	DDX_Check(pDX, IDC_EC_ENABLESCANGRID, C.EnableScanGrid);
	DDX_Text(pDX, IDC_EC_FOCUSOFFSET, C.FocusOffset);
	DDV_MinMaxFloat(pDX, C.FocusOffset, -30.0f, 30.f);
	DDX_Text(pDX, IDC_EC_PLATEQUALITYWAIT, C.PlateQualityWait);
	DDV_MinMaxInt(pDX, C.PlateQualityWait, 0, 600);
	DDX_Check(pDX, IDC_EC_AUTOTUNE, C.AutoTuneFog);
}
