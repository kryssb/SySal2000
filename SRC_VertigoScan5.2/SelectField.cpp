...
CSelectField::CSelectField(void (*papply)(void *, bool), void *pcontext, IO_VS_Fragment2 *pfrag, CWnd* pParent /*=NULL*/)
	: CDialog(CSelectField::IDD, pParent), pApply(papply), pContext(pcontext), pFrag(pfrag)
{
	...
	m_XField = 0;
	m_YField = 0;
	m_ShowMicrotrackExtrapolations = FALSE;
	//}}AFX_DATA_INIT
}
...
void CSelectField::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_SF_SHOWEXTRAPOLATION, m_ShowMicrotrackExtrapolations);
	//}}AFX_DATA_MAP
}
...
void CSelectField::OnApplyN() 
{
	...
	UpdateData(FALSE);
	pApply(pContext, m_ShowMicrotrackExtrapolations);
}
