...
UINT CVertigoScanner::Init()
{
	...
	try
	{
		...
		/*
		ITracker4 *iNTk[2] = {0, 0};
		for (i = 0; i < 2; i++)
		{
			if (iTk[i]->QueryInterface(IID_ITracker4, (void **)&iNTk[i]) == S_OK)
			{
				int nmult = 0;
				iNTk[i]->GetMaxGrainsPerTrack(&nmult);
				if (nmult > GrainMultiplicity) GrainMultiplicity = nmult;
				}
			if (iNTk[i]) iNTk[i]->Release();
			}
		*/
		for (i = 0; i < 2; i++)
		{
		...
		if (!(pCSpace = (Cluster *)debug_GlobalAlloc(GMEM_FIXED, sizeof(Cluster) * C.MaxClusters + sizeof(t_ProcessVariables::t_Rec) * ProcessVariables.FocusSamples))) throw 1;
		if (!(pTSpace = (Track2 *)debug_GlobalAlloc(GMEM_FIXED, 2 * sizeof(Track2) * C.MaxTracks))) throw 2;
		if (!(pPSpace = (Grain *)debug_GlobalAlloc(GMEM_FIXED, 2 * sizeof(Grain) * C.MaxTracks * GrainMultiplicity))) throw 3;
		...
		else if (!(pCorrSpace = (BYTE *)debug_GlobalAlloc(GMEM_FIXED, 2 * FitCorrectionDataSize * C.MaxTracks))) throw 4;
		...
		if (!(pVSpace = (VS_View2 *)debug_GlobalAlloc(GMEM_FIXED, sizeof(VS_View2) * C.XFields() * C.YFields()))) throw 5;
		if (!(pLSpace = (VS_View2::t_LayerInfo *)debug_GlobalAlloc(GMEM_FIXED, 2 * sizeof(VS_View2::t_LayerInfo) * C.XFields() * C.YFields() * C.VLayers))) throw 6;
		...
		if (!(ProcessVariables.FocusAIS.pImages = (AsyncProcessedImage *)debug_GlobalAlloc(GMEM_FIXED, sizeof(AsyncProcessedImage) * ProcessVariables.FocusSamples))) throw 13;
		if (!(ProcessVariables.FocusAIS.pImages[0].pClusters = (Cluster *)debug_GlobalAlloc(GMEM_FIXED, sizeof(Cluster) * C.MaxClusters * ProcessVariables.FocusSamples))) throw 14;
		...
		if (!(ProcessVariables.pAIS = (AsyncImageSequence *)debug_GlobalAlloc(GMEM_FIXED, sizeof(AsyncImageSequence) * (1 + ProcessVariables.FieldDelay)))) throw 7;
		for (i = 0; i < (1 + ProcessVariables.FieldDelay); i++)
		{
			ProcessVariables.pAIS[i].pImages = 0;
			ProcessVariables.pAIS[i].ImageCount = 0;
			}
		if (!(ProcessVariables.pFieldSideRec = (t_ProcessVariables::t_FieldSideRec *)debug_GlobalAlloc(GMEM_FIXED, sizeof(t_ProcessVariables::t_FieldSideRec) * (1 + ProcessVariables.FieldDelay)))) throw 8;
		if (!(ProcessVariables.pAZT = (AsyncZScanTrajectory *)debug_GlobalAlloc(GMEM_FIXED, sizeof(AsyncZScanTrajectory) * (1 + ProcessVariables.FieldDelay)))) throw 9;
		for (i = 0; i < (1 + ProcessVariables.FieldDelay); i++)
		...
		for (i = 0; i < (1 + ProcessVariables.FieldDelay); i++)
		{
			...
			if (!(ProcessVariables.pAIS[i].pImages = (AsyncProcessedImage *)debug_GlobalAlloc(GMEM_FIXED, sizeof(AsyncProcessedImage) * ProcessVariables.pAIS[i].ImageCount))) throw 10;
			for (j = 0; j < ProcessVariables.pAIS[i].ImageCount; j++) ProcessVariables.pAIS[i].pImages[j].pClusters = 0;
			if (!(ProcessVariables.pAIS[i].pImages[0].pClusters = (Cluster *)debug_GlobalAlloc(GMEM_FIXED, sizeof(Cluster) * C.MaxClusters * ProcessVariables.pAIS[i].ImageCount))) throw 11;
			...
			if (!(ProcessVariables.pAZT[i].pZSamples = (AsyncZSample *)debug_GlobalAlloc(GMEM_FIXED, sizeof(AsyncZSample) * ProcessVariables.pAZT[i].SampleCount))) throw 12;
			}
		if (!(ProcessVariables.FocusAZT.pZSamples = (AsyncZSample *)debug_GlobalAlloc(GMEM_FIXED, sizeof(AsyncZSample) * C.FocusZSamples()))) throw 14;
	...
	try
	{
		...
		if (ProcessVariables.pAIS)
		{
			for (i = 0; i < (1 + ProcessVariables.FieldDelay); i++)
			{
				...
				{
					int j;
					if (ProcessVariables.pAIS[i].pImages[0].pClusters) debug_GlobalFree(ProcessVariables.pAIS[i].pImages[0].pClusters);
		...
		{
			if (ProcessVariables.FocusAIS.pImages[0].pClusters) debug_GlobalFree(ProcessVariables.FocusAIS.pImages[0].pClusters);
		...
		{
			for (i = 0; i < (1 + ProcessVariables.FieldDelay); i++)
				if (ProcessVariables.pAZT[i].pZSamples) debug_GlobalFree(ProcessVariables.pAZT[i].pZSamples);
...
// KRYSS: THICKNESS?	
	...
	else if (IsTop)
	{
		...
		if (v <= C.VInactiveLayers) C.TunedTopStep += C.DZLayers(IsTop, false) * 0.5f;
		if (i >= C.VLayers) C.TunedTopStep += C.DZLayers(IsTop, false) * 0.5f;
	...
	else
	{
		...
		if (v <= C.VInactiveLayers) C.TunedBottomStep += C.DZLayers(IsTop, false) * 0.5f;
		if (i >= C.VLayers) C.TunedBottomStep += C.DZLayers(IsTop, false) * 0.5f;
...
#ifdef _DEBUG
	FILE *f = fopen("c:\\acq\\vs5.2.focus.txt", "a+t");
	fprintf(f, "%s\t%f\n", IsTop ? "Top" : "Bottom", IsTop ? C.TunedTopStep : C.TunedBottomStep);
	fclose(f);
#endif
// END KRYSS
...
#ifdef _DEBUG
	FILE *f = fopen("c:\\acq\\vs5.2.focus.txt", "a+t");
	fprintf(f, "Valley\t%s\t%f\t%f\t%f\n", IsTop ? "Top" : "Bottom", *pTopZ, *pBottomZ, *pTopZ - *pBottomZ);
	fclose(f);
#endif
// END KRYSS
			}
...
#else
	if (SingleViewMode || SeekSurface(true, v, SEEK_SURFACE_ADJUST, &top, &bottom, &valid) != VSERR_OK) 
	{
		FocusEndNotify();
		return VSSCAN_TERMINATED | VSSCAN_NOTOPFOCUS | VSSCAN_NOBOTTOMFOCUS;
		}
...
UINT CVertigoScanner::ScanSingleView(float startx, float starty, bool moveafterscan, float moveafterx, float moveaftery, bool retry)
{
	...
	for (s = 0; s < 2; s++)
	{
		...
		ProcessVariables.ValidLayers = lastlayer + 1 - firstlayer;
		if (ProcessVariables.ValidLayers < C.MinValidLayers || (ProcessVariables.ValidLayers < C.VLayers && (firstlayer <= C.VInactiveLayers || lastlayer >= ProcessVariables.pAIS[s].ImageCount - 1)))
		{
