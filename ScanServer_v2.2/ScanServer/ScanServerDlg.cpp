...
CScanServerDlg::CScanServerDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CScanServerDlg::IDD, pParent), BkgndBrush(RGB(255, 255, 255)), iCOMSrv(0)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
	HMsgSync = CreateEvent(NULL, FALSE, FALSE, NULL);
	CoInitializeEx(NULL, COINIT_MULTITHREADED);

	if (CoCreateInstance(CLSID_ScanServerMessageRouter, NULL, CLSCTX_INPROC_SERVER, IID_IScanServerMessageRouter, (void **)&iCOMSrv) != S_OK)
	{
		MessageBox("Can't create COM Message Router!\r\nAborting.", "COM Error", MB_ICONERROR);
		PostQuitMessage(0);
		return;
	}
	ScanServerMessageRoutingMap Map;
	Map.pContext = (void *)this;
	Map.pConnect = Connect;
	Map.pDisconnect = Disconnect;
	Map.pReceive = Receive;
	Map.pSend = Send;
	if (iCOMSrv->SetMessageRoutingMap(&Map) != S_OK)
	{
		MessageBox("Can't set the message routing map!\r\nAborting.", "Unknown Error", MB_ICONERROR);
		PostQuitMessage(0);
		return;
	}
	ISySalObject *iOb = 0;
	iCOMSrv->QueryInterface(IID_ISySalObject, (void **)&iOb);
	pNETSrv = SySal::DAQSystem::ScanServer::Create(this, CreateObjectInWndThread, DeleteObjectInWndThread, AddString, iOb);
	iOb->Release();
...
{
	if (pNETSrv && pChannel) SySal::DAQSystem::ScanServer::UnregisterChannelAndServer(pChannel, pNETSrv);
...
HRESULT CScanServerDlg::Send(void *pOwner, BYTE *pMessage)
{
	CScanServerDlg *This = (CScanServerDlg *)pOwner;
	return This->pNETSrv->Send((ScanSrvMessage *)pMessage) ? S_OK : S_FALSE;
}
	
HRESULT CScanServerDlg::Receive(void *pOwner, BYTE *pMessageSpace)
{
	CScanServerDlg *This = (CScanServerDlg *)pOwner;
	return This->pNETSrv->Receive((ScanSrvMessage *)pMessageSpace) ? S_OK : S_FALSE;
}
...
{
	// TODO: Add your message handler code here and/or call default
	if (MessageBox("Are you really sure you want to close?", "Warning", MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON2) != IDYES) return;
...
#include "..\ScanServerMessageRouter\ScanServerMessageRouter_i.c"
