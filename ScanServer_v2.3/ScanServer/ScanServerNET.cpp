...
{	
	...
	{
		ScanZoneStartPresetSlope *p = new ScanZoneStartPresetSlope;
		pMessage = p;
		p->OpCode = SCANSRV_SCAN_ZONE_START_SLOPE_PRESET;
		p->Size = sizeof(ScanZoneStartPresetSlope);
		p->Id.Part[0] = m_PlateId.Part0;
		p->Id.Part[1] = m_PlateId.Part1;
		p->Id.Part[2] = zone->Series;
		p->Id.Part[3] = 0;
		...
		for (i = 0; i < 255 && i < zone->Outname->Length; i++)
			p->OutName[i] = zone->Outname->Chars[i];
		p->OutName[i] = 0;
		p->SlopeX = zone->PresetSlope.X;
		p->SlopeY = zone->PresetSlope.Y;
		p->SlopeAccX = zone->PresetSlopeAcc.X;
		p->SlopeAccY = zone->PresetSlopeAcc.Y;

		sprintf(msgstr, "\r\nScanning zone %d/%d/%d/%d MinX: %.1f MaxX: %.1f MinY: %.1f MaxY: %.1f SX: %.4f SY: %.4f AccX: %.4f AccY: %.4f ... ", 
			p->Id.Part[0], p->Id.Part[1], p->Id.Part[2], p->Id.Part[3],
			p->MinX, p->MaxX, p->MinY, p->MaxY, p->SlopeX, p->SlopeY, p->SlopeAccX, p->SlopeAccY);
	}
	m_IsBusy = true;
...
{	
	...
	{
		ScanZoneStartAndPreloadNextPresetSlope *p = new ScanZoneStartAndPreloadNextPresetSlope;
		pMessage = p;
		p->OpCode = SCANSRV_SCAN_ZONE_START_PRELOAD_NEXT_SLOPE_PRESET;
		p->Size = sizeof(ScanZoneStartAndPreloadNextPresetSlope);
		p->Id.Part[0] = m_PlateId.Part0;
		p->Id.Part[1] = m_PlateId.Part1;
		p->Id.Part[2] = zone->Series;
		p->Id.Part[3] = 0;
