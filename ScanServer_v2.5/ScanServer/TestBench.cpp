...
void CTestBench::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_TB_IMDUMP_ID1, m_ImID1);
	DDX_Text(pDX, IDC_TB_IMDUMP_ID2, m_ImID2);
	DDX_Text(pDX, IDC_TB_IMDUMP_ID3, m_ImID3);
	DDX_Text(pDX, IDC_TB_IMDUMP_ID4, m_ImID4);
	DDX_Text(pDX, IDC_TB_IMDUMP_X, m_ImPosX);
	DDX_Text(pDX, IDC_TB_IMDUMP_Y, m_ImPosY);
	DDX_Text(pDX, IDC_TB_IMDUMP_SX, m_ImSlopeX);
	DDX_Text(pDX, IDC_TB_IMDUMP_SY, m_ImSlopeY);
	DDX_Text(pDX, IDC_TB_IMDUMP_OUTPATH, m_ImOutPath);
}
...
//		UpdateData(FALSE);
	...
	else if (SendDlgItemMessage(IDC_TB_IMAGEDUMPREQ, BM_GETCHECK) == BST_CHECKED)
	{
		SySal::DAQSystem::Scanning::ImageDumpRequest imdumpreq;
		bool ret = false;
		try
		{
			imdumpreq.Id.Part0 = m_ImID1;
			imdumpreq.Id.Part1 = m_ImID2;
			imdumpreq.Id.Part2 = m_ImID3;
			imdumpreq.Id.Part3 = m_ImID4;
			imdumpreq.Position.X = m_ImPosX;
			imdumpreq.Position.Y = m_ImPosY;
			imdumpreq.Slope.X = m_ImSlopeX;
			imdumpreq.Slope.Y = m_ImSlopeY;
			imdumpreq.OutputPath = gcnew System::String(m_ImOutPath);
			ret = SS->ImageDump(imdumpreq);
			m_Exception = "";
		}
		catch (System::Exception ^x)
		{
