...
OBJECT_ENTRY(CLSID_SmartTracker, CSmartTrackerClass)
END_OBJECT_MAP()
...
STDAPI DllRegisterServer(void)
{
	...
	try
	{
		...
		MyKey.Close();
		if (MyKey.Create(HKEY_LOCAL_MACHINE, "Software\\SySal2\\Classes\\SmartTracker", REG_NONE, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE) != ERROR_SUCCESS) throw 3;
		MyKey.Close();
		MyKey.Open(HKEY_LOCAL_MACHINE, "Software\\SySal2\\Classes\\SmartTracker", KEY_READ | KEY_WRITE);
		char CLSIDString[64];
		wsprintf(CLSIDString, "{%08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X}", 
			CLSID_SmartTracker.Data1, CLSID_SmartTracker.Data2, CLSID_SmartTracker.Data3, 
			CLSID_SmartTracker.Data4[0], CLSID_SmartTracker.Data4[1], CLSID_SmartTracker.Data4[2], CLSID_SmartTracker.Data4[3], 
			CLSID_SmartTracker.Data4[4], CLSID_SmartTracker.Data4[5], CLSID_SmartTracker.Data4[6], CLSID_SmartTracker.Data4[7]);
	...
	catch (...)
	{
		...
		MyKey.RecurseDeleteKey("Software\\SySal2\\Classes\\SmartTracker");
		MyKey.Close();
		};
