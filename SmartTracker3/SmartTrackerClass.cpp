...
const int ConfigDataN = 14 + (3 + MAX_TRIGGERS) * MAX_TRIALS;
...
/* two very useful macros */
#define GETDIRANDTOL(x) \
		{\
		__asm { ftst };\
		__asm { fnstsw ax };\
		__asm { and ax, 0dh };\
		__asm { jz NonZeroSlope_ ## x };\
		__asm { fst dword ptr [DirY] };\
		__asm { fst dword ptr [dirtol] };\
		__asm { fld1 };\
		__asm { fst dword ptr [DirX] };\
		__asm { jmp ExitDirAndTol_ ## x };\
		NonZeroSlope_ ## x: __asm {	fld st(0) };\
		__asm {	fld1 };\
		__asm {	fdivr };\
		__asm {	fld st(0) };\
		__asm {	fmul dword ptr [SlopeX] };\
		__asm {	fstp dword ptr [DirX] };\
		__asm {	fmul dword ptr [SlopeY] };\
		__asm {	fstp dword ptr [DirY] };\
		__asm {	fmul dword ptr [avgdz] };\
		__asm {	fmul st(0), st(0) };\
		__asm {	fld dword ptr [aligntol] };\
		__asm {	fmul st(0), st(0) };\
		__asm {	fadd };\
		__asm {	fsqrt };\
		__asm {	fst dword ptr [dirtol] };\
...
#define GETDIRANDTOLLOADSLOPE(x) \
	__asm { fld dword ptr [Slope] }; \
	GETDIRANDTOL(x)
/* end macros */
...
STDMETHODIMP CSmartTrackerClass::GetTracks(Track *pSpace, int *pMaxNumber, int StartLayer, int EndLayer)
{
	...
	float dirtol, aligntol;
	int it, ib, ir;
	float idz, ldz, cbz;

	TrackerData Z = T;
	float idxcell = 1.0f / T.DxCell;
	float idycell = 1.0f / T.DyCell;
	float cellminx = T.Win.Xmin;
	float cellminy = T.Win.Ymin;
...
#define MAX_GRAINS_IN_TRACK 1024

	IntGrain *pTempGrains[MAX_GRAINS_IN_TRACK];

	float avgdz = (Z.ZLayer[StartLayer] - Z.ZLayer[EndLayer]) / (EndLayer - StartLayer);
			
	float tsl = 10.0f * (Z.NptMinV - Z.NptMin01) / (Z.NptMin01 - Z.NptMinH);
	float tinf = (float)(Z.NptMinH);
	float td = (float)(Z.NptMinV - Z.NptMinH);

	int trcount = Z.TrialCount;
	Trial *ptrials = Z.Trials;
	int *ptriggerlayers;
	int triggercount;
	Cell *ctp, *cbp;
	float *ZLayer = T.ZLayer;
	int CellSpace = (int)(void *)Z.CellSpace;
	int CellsInLayer = Z.CellsInLayer;
	int CellsInLayerSize = CellsInLayer * sizeof(Cell);
	float MaxSlope = T.MaxSlope;
	int nx = T.CellNumX;
	int ny = T.CellNumY;
	aligntol = T.AlignTol;
	int tgrain, bgrain;
	IntGrain *ptgrain, *pbgrain;
...
LoopTrials:
	__asm 
	{
		...
		cmp ecx, [trcount]
		jge EndTrials
		shl ecx, 6							; the size of each trial is 64 bytes
		add ecx, [ptrials]
		mov eax, [ecx]Trial.TopLayer
		cmp eax, [StartLayer]
		jl LoopTrials
		mov edi, [ecx]Trial.BottomLayer
		cmp edi, [EndLayer]
		jg LoopTrials
		mov ebx, ecx
		add ebx, 0ch
		mov [ptriggerlayers], ebx			; ptriggerlayers points to the trigger layers for this trial
		mov ebx, [ecx]Trial.TriggerCount
		mov [triggercount], ebx

		mov esi, eax
		shl eax, 2							; the size of each ZLayer is 4 bytes
		mov ecx, [ZLayer]
		mov ebx, [eax + ecx]
		mov [tz], ebx
		fld1
		...
		fld dword ptr [tz]
		shl eax, 2							; the size of each ZLayer is 4 bytes
		fsub dword ptr [eax + ecx]		
		fdivr st(0), st(1)


		mov eax, esi
		mov ebx, [CellsInLayerSize]
		mov ecx, [CellSpace]
		fst dword ptr [idz]					; get the result of the division
		mul ebx
		add eax, ecx
		mov [ctp], eax
		mov eax, edi
...
NonMinIxLess:
	__asm
	{
		...
		inc eax
		cmp eax, [nx]
		jl NonMaxIxGreaterEqual
		mov eax, [nx]
		dec eax
		};

NonMaxIxGreaterEqual:
	__asm
	{
		mov [maxix], eax
		};
...
NonMinIyLess:
	__asm
	{
		...
		inc eax
		cmp eax, [ny]
		jl NonMaxIyGreaterEqual
		mov eax, [ny]
		dec eax
		};

NonMaxIyGreaterEqual:
	__asm
	{
		mov [maxiy], eax
		mov eax, ecx
		...
		add eax, [ix]
		add eax, [ctp]
		mov edi, [eax]						; edi points to the current grain
		mov ecx, [eax + 4]					; ecx contains the number of grains
		cmp ecx, 0
		jle LoopIy
		};
...
LoopOtherGrain:
	__asm
	{
		fld dword ptr [idz]
		mul [nx]
		fld dword ptr [edi]IntGrain.Xcm
		add eax, ebx
		add eax, [cbp]
		mov esi, [eax]						; esi points to the current grain
		mov ebx, [eax + 4]					; ebx contains the number of grains
		cmp ebx, 0
		jle LoopIiy
		fsub [esi]IntGrain.Xcm
		fmul
		fst dword ptr [SlopeX]
		fmul st(0), st(0)
		fld dword ptr [edi]IntGrain.Ycm
		fsub [esi]IntGrain.Ycm
		fmul [idz]
		fst dword ptr [SlopeY]				; Slope.X and Slope.Y are now computed
		fmul st(0), st(0)
		fadd 
		fsqrt
		fst dword ptr [Slope2]				; Slope2 is now computed
		fcom dword ptr [MaxSlope]
		fnstsw ax
		and ax, 0dh
		jz SearchAlignedGrains
		add esi, 32							; IntGrain size is assumed to be 32
		dec ebx
		jnz LoopOtherGrain
		mov eax, [iiy]
		inc eax
...
LoopTrigger:
	__asm
	{
		...
		fld st(1)
		fld st(1)
		fmul dword ptr [esi]IntGrain.Xcm
		fstp st(4)
		fmul dword ptr [edi]IntGrain.Xcm
		fadd st(0), st(3)
		fst dword ptr [ExpX]
		fstp st(4)
		fmul dword ptr [esi]IntGrain.Ycm
		fstp st(2)
		fmul dword ptr [edi]IntGrain.Ycm
		fadd
		fst dword ptr [ExpY]
		fsub dword ptr [cellminy]
		fmul dword ptr [idycell]
		fistp dword ptr [eiy]
		fsub dword ptr [cellminx]
		fmul dword ptr [idxcell]
		fistp dword ptr [eix]
...
/*
	for (itr = 0; itr < Z.TrialCount; itr++)
	{
		Trial tr = Z.Trials[itr];
							...
							for (;pB < pBE; pB++)
							{
								...
								if (Slope2 < MaxSlope)
								{
									...
									for (ir = 0; ir < tr.TriggerCount; ir++)
									{
										...
										eix = (int)((ExpX - Z.Win.Xmin) / Z.DxCell);
										eiy = (int)((ExpY - Z.Win.Ymin) / Z.DyCell);
										IntGrain *pTrg = CellSpace[(TriggerLayer * ny + eiy) * nx + eix].FindGrain(ExpX, ExpY, DirX, DirY, dirtol, Z.AlignTol);
										if (pTrg)
										{
											memset(pTempGrains, 0, Z.Layer * sizeof(Grain *));
											float dz;
											int seqlength = 3;
											...
											for (lay = tr.TopLayer + 1; lay < TriggerLayer; lay++)
											{
												...
												if (pTempGrains[lay] = FindGrain(ExpX, ExpY, CellSpace + lay * Z.CellsInLayer, DirX, DirY, dirtol)) seqlength++;
												};
											...
											for (lay = (int)tr.TopLayer - 1; lay >= 0; lay--)
											{
												...
												if (pTempGrains[lay] = FindGrain(ExpX, ExpY, CellSpace + lay * Z.CellsInLayer, DirX, DirY, dirtol))
												{
...
IntGrain *CSmartTrackerClass::FindGrain(float expx, float expy, Cell *pCellLayer, float dirx, float diry, float dirtol)
{
	...
	if (eix >= 0 && eix < T.CellNumX && eiy >= 0 && eiy < T.CellNumY)
	{
		...
		{
			if (fix < 0.0f && eix > 0)
				return pCellLayer[eiy * T.CellNumX + eix - 1].FindGrain(expx, expy, dirx, diry, dirtol, T.AlignTol);
		...
		{
			if (fiy < 0.0f && eiy > 0)
				return pCellLayer[(eiy - 1) * T.CellNumX + eix].FindGrain(expx, expy, dirx, diry, dirtol, T.AlignTol);
	...
	for (i = j = 0; i < ntracks; i++)
		if (pspace[i].Valid)
		{
			...
			for (k = 0; k < N.Grains; k++)
			{
				...
				a += pG->Area;
				N.pGrains[k].Z = ((IntTrackGrain *)O.pGrains)[k].T.Z;
				N.pGrains[k].X = pG->Xcm;
				N.pGrains[k].Y = pG->Ycm;
				}
			N.AreaSum = a;
			}
	return j;
