...
const int ConfigDataN = 15 + (3 + MAX_TRIGGERS) * MAX_TRIALS;
...
STDMETHODIMP CSmartTrackerClass::GetClassName(ConnectionName name)
{
	...
	strncpy(name, "Smart Tracker 4", SYSAL_MAX_CONN_NAME_LEN);
	return S_OK;
}
...
HRESULT CSmartTrackerClass::GetTracks2(Track2 *pSpace, int *pMaxNumber, int StartLayer, int EndLayer)
{
	...
	for (p = 1; p < T.Processors; p++)
		for (i = 0; i < ThreadInfo[p].Found; i++)
		{
			...
			pSpace[j].Field = ThreadInfo[p].pSpace[i].Field;
			pSpace[j].Grains = ThreadInfo[p].pSpace[i].Grains;
			pTemp = pSpace[j].pGrains;
			pSpace[j].pGrains = ThreadInfo[p].pSpace[i].pGrains;
			ThreadInfo[p].pSpace[i].pGrains = pTemp;
			found++;
			}
