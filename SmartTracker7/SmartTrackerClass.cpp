...
STDMETHODIMP CSmartTrackerClass::GetClassName(ConnectionName name)
{
	...
	strncpy(name, "Smart Tracker 7", SYSAL_MAX_CONN_NAME_LEN);
	return S_OK;
}
...
/*
	FILE *fsmtg = fopen("c:\\acq\\smt7g.txt", "a+t");
	FILE *fsmtd = fopen("c:\\acq\\smt7d.txt", "a+t");
*/
										...
										if (pTrg)
										{
											...
											if ((float)seqlength > (tinf + td / (1.0f + tsl * SlopeS)))
											{
												...
												if (T.CheckGrainSpacing)
												{
													...
													if ((float)fillcount <= (tinf + td / (1.0f + tsl * SlopeS))) continue;// break;
													}
