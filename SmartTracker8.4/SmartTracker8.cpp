...
STDAPI DllRegisterServer(void)
{
	...
	try
	{
		...
		MyKey.Close();
		if (MyKey.Create(HKEY_LOCAL_MACHINE, "Software\\SySal2\\Classes\\SmartTracker8", REG_NONE, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE) != ERROR_SUCCESS) throw 3;
		MyKey.Close();
		MyKey.Open(HKEY_LOCAL_MACHINE, "Software\\SySal2\\Classes\\SmartTracker8", KEY_READ | KEY_WRITE);
		char CLSIDString[64];
	...
	catch (...)
	{
		...
		MyKey.RecurseDeleteKey("Software\\SySal2\\Classes\\SmartTracker8");
		MyKey.Close();
		};
