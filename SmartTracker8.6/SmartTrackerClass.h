...
public:
	...
	CSmartTrackerClass()
	{
		...
		MaxProcessors = sysinfo.dwNumberOfProcessors;
		ConfigMaxProcessors_backup = 0;
		ThreadInfo = new ActivationRecord[MaxProcessors];
		HEvs = new HANDLE[MaxProcessors];
	...
	virtual ~CSmartTrackerClass()
	{
		...
		if (T.GrainSpace)
		{	CoTaskMemFree(T.GrainSpace);
			T.GrainSpace = 0;
			};
		if (T.ZLayer)
		{	CoTaskMemFree(T.ZLayer);
			T.ZLayer = 0;
			};
		if (T.LayerGrains)
		{	CoTaskMemFree(T.LayerGrains);
			T.LayerGrains = 0;
			};
		if (T.CellSpace)
		{	CoTaskMemFree(T.CellSpace);
			T.CellSpace = 0;
			};
		if (T.pDeltaHisto2d)
		{
			CoTaskMemFree(T.pDeltaHisto2d);
			T.pDeltaHisto2d = 0;
			}
		if (T.pDeltaData)
		{
			CoTaskMemFree(T.pDeltaData);
			T.pDeltaData = 0;
...
protected:
	...
	int MaxProcessors;
	int ConfigMaxProcessors_backup;

	ActivationRecord *ThreadInfo;
	HANDLE *HEvs;
