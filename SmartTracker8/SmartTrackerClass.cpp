...
const int ConfigDataN = 24 + (3 + MAX_TRIGGERS) * MAX_TRIALS;
...
QueryDataInfo ConfigDataQDI[ConfigDataN] =
{
	...
	{"LongitTolMultiplier",		ST_FLOAT,		STRUCT_OFFSET(TrackerData, LongitTolMultiplier),	"1.0"},
	...
	{"TransverseSigmaLimit",	ST_FLOAT,	STRUCT_OFFSET(TrackerData, TransverseSigmaLimit),	"0.2"},	
...
STDMETHODIMP CSmartTrackerClass::SetConfig(SySalConfig *pConfig)
{
	...
	if (!(T.CellSpace = (Cell *)GlobalAlloc(GMEM_FIXED, sizeof(Cell) * (T.CellsInLayer)*(T.LayerNum))))
   		return S_FALSE;
	if (T.pDeltaData) GlobalFree(T.pDeltaData);
	if (!(T.pDeltaData = (float *)GlobalAlloc(GMEM_FIXED, (4 * sizeof(float) + sizeof(IntCluster **)) * T.MaxGrains)))
		return S_FALSE;
	else
	{
		T.pX = T.pDeltaData;
...
STDMETHODIMP CSmartTrackerClass::GetClassName(ConnectionName name)
{
	...
	strncpy(name, "Smart Tracker 8.1", SYSAL_MAX_CONN_NAME_LEN);
	return S_OK;
}
...
//						PGrain->Ycm = PClus->Y + DispY;
						if (PClus->Area < 0)
						{
							...
							PGrain->Xcm = PClus->X * L1X + L0X;
							PGrain->Ycm = PClus->Y * L1Y + L0Y;
							PGrain->Area = -PClus->Area;
							if (PGrain->Area <= 0)
							{
								::MessageBox(0, "Here - A!", "Debug Info", MB_OK);
								volatile uy = 0;
								uy = 1;
								}
							}
						else
						{
							...
							if (PGrain->Area <= 0)
							{
								::MessageBox(0, "Here - B!", "Debug Info", MB_OK);
								volatile uy = 0;
								uy = 1;
								}
							}
			...
			{	
				...
				{	
					...
					{	
						...
						if (PGrain->Area <= 0)
						{
							::MessageBox(0, "Here - C!", "Debug Info", MB_OK);
							volatile uy = 0;
							uy = 1;
							}
						PClus = PClus->NextFree;
...
void CSmartTrackerClass::GetDirAndTol(float SlopeX, float SlopeY, float avgdz, float *pDirX, float *pDirY, float *pdirtol)
{
	...
	else
	{
		*pDirX = SlopeX / slope2;
		*pDirY = SlopeY / slope2;
		*pdirtol = T.AlignTol + avgdz * slope2 * T.LongitTolMultiplier;
	}
};
...
{ \
	...
	{ \
		DirX = SlopeX * ISlopeS; \
		DirY = SlopeY * ISlopeS; \
		dirtol = T.AlignTol + SlopeS * avgdz * T.LongitTolMultiplier; \
		}; \
	}								
...
{
	if (T.pDeltaHisto2d) GlobalFree(T.pDeltaHisto2d);
	T.pDeltaHisto2d = 0;
	...
	return T.pDeltaHisto2d = (int *)GlobalAlloc(GMEM_FIXED, sizeof(int) * T.DeltaBinsX * T.DeltaBinsY);
}
