	...
	{		
		...
		{	
			...
			for (i = 0; i < SheetsCount; i++)
			{
				...
				else if (pInput->Hdr.InfoType != (BULK_HEADER_BYTES | BULK_TRACK_SECTION) || pInput->Hdr.HeaderFormat != BULK_HEADER_TYPE) 
				{
					WorkString.Format("%d", ++TotalErrors);
					m_ErrorCount.SetWindowText(WorkString);
					RM.EndAdding();
				...
				TempLayer.RelevantZs.TopExt = pInput->BTks.Hdr.RelevantZs.TopExt;
				TempLayer.RelevantZs.TopInt = pInput->BTks.Hdr.RelevantZs.TopInt;
				TempLayer.RelevantZs.BottomInt = pInput->BTks.Hdr.RelevantZs.BottomInt;
				TempLayer.RelevantZs.BottomExt = pInput->BTks.Hdr.RelevantZs.BottomExt;
				...
				TempLayer.RTCount[0] = pInput->BTks.Hdr.TCount[0];
				TempLayer.RTCount[1] = pInput->BTks.Hdr.TCount[1];
				...
				for (s = 0; s < 2; s++)
					...
					{
						TempLayer.pRawTracks[s][j].Field = pInput->BTks.pTracks[s][j].Field;
						TempLayer.pRawTracks[s][j].PointsN = pInput->BTks.pTracks[s][j].PointsN;
						TempLayer.pRawTracks[s][j].Intercept = pInput->BTks.pTracks[s][j].Intercept;
						TempLayer.pRawTracks[s][j].Slope = pInput->BTks.pTracks[s][j].Slope;
						TempLayer.pRawTracks[s][j].Sigma = pInput->BTks.pTracks[s][j].Sigma;
						TempLayer.pRawTracks[s][j].FirstZ = pInput->BTks.pTracks[s][j].FirstZ;
						TempLayer.pRawTracks[s][j].LastZ = pInput->BTks.pTracks[s][j].LastZ;
						};
