...
template <class T> class CellArray
{
	...
	public:
		...
		CellArray(int Count, T *pItems, float ReferenceZ, float XYTol) : XCells(0), YCells(0), pCells(0)
		{
			...
			if (Count > 0)
			{
				...
				XCells = floor((Max.X - Min.X) / DCell + 1.);
				YCells = floor((Max.Y - Min.Y) / DCell + 1.);
				...
				for (i = 0; i < Count; i++)
					pCells[pI[i] = floor((pV[i].X - Min.X) / DCell) + (floor((pV[i].Y - Min.Y) / DCell) * XCells)].Count++;
				for (i = 0; i < XCells * YCells; i++)
