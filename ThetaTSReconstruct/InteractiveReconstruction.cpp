...
#undef DISABLE_EXCEPTION_INTERCEPTION
...
bool CInteractiveReconstruction::LoadTracks(FILE *f, int N, CInteractiveReconstruction::AlignmentData *pData)
{
	try
	{
		...
		for (i = 0; i < Plates; i++)
		{
			...
			if (pTracks->Hdr.InfoType != (CS_SS_HEADER_BYTES | CS_SS_TRACK_SECTION) || pTracks->Hdr.HeaderFormat != CS_SS_HEADER_TYPE) 
			{
			...
			TempLayer.RelevantZs.TopExt = pTracks->STks.Hdr.RelevantZs.TopExt;
			TempLayer.RelevantZs.TopInt = pTracks->STks.Hdr.RelevantZs.TopInt;
			TempLayer.RelevantZs.BottomInt = pTracks->STks.Hdr.RelevantZs.BottomInt;
			TempLayer.RelevantZs.BottomExt = pTracks->STks.Hdr.RelevantZs.BottomExt;
			...
			TempLayer.RTCount[0] = pTracks->STks.Hdr.LCount;
			...
			for (j = 0; j < TempLayer.RTCount[0]; j++)
			{
				...
				TempLayer.pRawTracks[0][j].PointsN = pTracks->STks.pLinked[j].PointsN;
				TempLayer.pRawTracks[0][j].Intercept = pTracks->STks.pLinked[j].Intercept;
				TempLayer.pRawTracks[0][j].Slope = pTracks->STks.pLinked[j].Slope;
				TempLayer.pRawTracks[0][j].Sigma = pTracks->STks.pLinked[j].Sigma;
				TempLayer.pRawTracks[0][j].FirstZ = pTracks->STks.Hdr.RelevantZs.TopInt;
				TempLayer.pRawTracks[0][j].LastZ = pTracks->STks.Hdr.RelevantZs.BottomInt;
				};
