...
/* NEW */
	...
	float MinZ, MaxZ, MaxSlope;
	MinZ = pBottomSegArray[0].LastZ;
	MaxZ = pBottomSegArray[0].FirstZ;
	MaxSlope = hypot(pBottomSegArray[0].Slope.X, pBottomSegArray[0].Slope.Y);
	{
		int i;
		for (i = 1; i < BottomSegCount; i++)
		{
			...
			float s = hypot(pBottomSegArray[i].Slope.X, pBottomSegArray[i].Slope.Y);
		...
		for (i = 0; i < TopSegCount; i++)
		{
			...
			float s = hypot(pTopSegArray[i].Slope.X, pTopSegArray[i].Slope.Y);
			if (MaxSlope < s) MaxSlope = s;
			};		
		};
...
/* NEW */
		...
		CellArray<TS_Segment> CA(TopSegCount, pTopSegArray, (MinZ + MaxZ) * 0.5f, PosTol + MaxSlope * (MaxZ - MinZ));
...
/* NEW */
			...
			{
				i2 = CA.ExternalLock.pItems[locki2] - pTopSegArray;
/* END NEW */
...
{
//	Vector2 I(T1->pBottomEnd->Intercept - T2->pBottomEnd->Intercept - (T1->pBottomEnd->Intercept.Z - T2->pBottomEnd->Intercept.Z) * T2->pBottomEnd->Slope);
	Vector2 I(T1->Intercept - T2->Intercept - (T1->Intercept.Z - T2->Intercept.Z) * T2->Slope);
	Vector2 J;
//	Vector2 S(T1->pBottomEnd->Slope - T2->pBottomEnd->Slope);
	...
	{
		W = (I.X * S.X + I.Y * S.Y) / N;
//		Vector Pos = 0.5 * (T1->pBottomEnd->Intercept - W * T1->pBottomEnd->Slope + T2->pBottomEnd->Intercept + (T1->pBottomEnd->Intercept.Z - T2->pBottomEnd->Intercept.Z - W) * T2->pBottomEnd->Slope);
...
//	if ((DDist = T1->pBottomEnd->LastZ - T2->pTopEnd->FirstZ + 2. * TSRD.Vertex_Overlength) > 0.) 
	...
	{
/*		
		I = T1->pBottomEnd->Intercept + (T1->pBottomEnd->LastZ + TSRD.Vertex_Overlength - T1->pBottomEnd->Intercept.Z) * T1->pBottomEnd->Slope - 
			T2->pTopEnd->Intercept - (T1->pBottomEnd->LastZ + TSRD.Vertex_Overlength - T2->pTopEnd->Intercept.Z) * T2->pTopEnd->Slope;
		J = T1->pBottomEnd->Intercept + (T2->pTopEnd->FirstZ - TSRD.Vertex_Overlength - T1->pBottomEnd->Intercept.Z) * T1->pBottomEnd->Slope - 
			T2->pTopEnd->Intercept - (T2->pTopEnd->FirstZ - TSRD.Vertex_Overlength - T2->pTopEnd->Intercept.Z) * T2->pTopEnd->Slope;
*/
...
/*
			pC->Pos = 0.5 * (T1->pBottomEnd->Intercept + (T1->pBottomEnd->LastZ + TSRD.Vertex_Overlength - 
				T1->pBottomEnd->Intercept.Z - W * DDist) * T1->pBottomEnd->Slope + 
				T2->pTopEnd->Intercept + (T2->pTopEnd->FirstZ - TSRD.Vertex_Overlength - 
				T2->pTopEnd->Intercept.Z + (1. - W) * DDist) * T2->pTopEnd->Slope);
*/
...
//	if ((DDist = T2->pBottomEnd->LastZ - T1->pTopEnd->FirstZ + 2. * TSRD.Vertex_Overlength) > 0.) 
	...
	{
/*
		I = T2->pBottomEnd->Intercept + (T2->pBottomEnd->LastZ + TSRD.Vertex_Overlength - T2->pBottomEnd->Intercept.Z) * T2->pBottomEnd->Slope - 
			T1->pTopEnd->Intercept - (T2->pBottomEnd->LastZ + TSRD.Vertex_Overlength - T1->pTopEnd->Intercept.Z) * T1->pTopEnd->Slope;
		J = T2->pBottomEnd->Intercept + (T1->pTopEnd->FirstZ - TSRD.Vertex_Overlength - T2->pBottomEnd->Intercept.Z) * T2->pBottomEnd->Slope - 
			T1->pTopEnd->Intercept - (T1->pTopEnd->FirstZ - TSRD.Vertex_Overlength - T1->pTopEnd->Intercept.Z) * T1->pTopEnd->Slope;
*/
...
/*
			pC->Pos = 0.5 * (T2->pBottomEnd->Intercept + (T2->pBottomEnd->LastZ + TSRD.Vertex_Overlength - 
				T2->pBottomEnd->Intercept.Z - W * DDist) * T2->pBottomEnd->Slope + 
				T1->pTopEnd->Intercept + (T1->pTopEnd->FirstZ - TSRD.Vertex_Overlength - 
				T1->pTopEnd->Intercept.Z + (2. - W) * DDist) * T1->pTopEnd->Slope);
*/
...
bool TSReconstructionManager::FitTrack(TS_Track *pTSTrack)
{
	...
	for (pSeg = pTSTrack->pBottomEnd; pSeg; pSeg = pSeg->pTopLink)
	{
		...
		if (pSeg->LastZ < pTSTrack->LastZ) pTSTrack->LastZ = pSeg->LastZ - a.PosAlign.D[2];
		Z = pSeg->LastZ - pTSReconstruction->Hdr.RefCenter.Z - a.PosAlign.D[2];
		Sz += Z;
		Sz2 += Z * Z;
		if (pSeg->FirstZ > pTSTrack->FirstZ) pTSTrack->FirstZ = pSeg->FirstZ - a.PosAlign.D[2];
		Z = pSeg->FirstZ - pTSReconstruction->Hdr.RefCenter.Z - a.PosAlign.D[2];
		Sz += Z;
		Sz2 += Z * Z;
		};
	...
	for (pSeg = pTSTrack->pBottomEnd; pSeg; pSeg = pSeg->pTopLink)
	{
		...
		OS.Z = 1.;
		S.X = (1. + a.PosAlign.M[0][0]) * OS.X + a.PosAlign.M[0][1] * OS.Y;
		S.Y = a.PosAlign.M[1][0] * OS.X + (1. + a.PosAlign.M[1][1]) * OS.Y;
		S.Z = OS.Z;
		P.X = (1. + a.PosAlign.M[0][0]) * OP.X + a.PosAlign.M[0][1] * OP.Y + a.PosAlign.D[0];
		P.Y = a.PosAlign.M[1][0] * OP.X + (1. + a.PosAlign.M[1][1]) * OP.Y + a.PosAlign.D[1];
	...
	for (pSeg = pTSTrack->pBottomEnd; pSeg; pSeg = pSeg->pTopLink)
	{
		...
		//T = pSeg->Intercept + (pSeg->LastZ - pSeg->Intercept.Z) * pSeg->Slope - pTSReconstruction->Hdr.RefCenter;
		T = P + (pSeg->LastZ - pSeg->Intercept.Z) * S - pTSReconstruction->Hdr.RefCenter - pTSTrack->Intercept - 
			(pSeg->LastZ - a.PosAlign.D[2] - pTSTrack->Intercept.Z) * pTSTrack->Slope;
...
//		T = pSeg->Intercept + (pSeg->FirstZ - pSeg->Intercept.Z) * pSeg->Slope - pTSTrack->Intercept - (pSeg->FirstZ - pTSTrack->Intercept.Z) * pTSTrack->Slope;
		T = P + (pSeg->LastZ - pSeg->Intercept.Z) * S - pTSReconstruction->Hdr.RefCenter - pTSTrack->Intercept - 
			(pSeg->FirstZ - a.PosAlign.D[2] - pTSTrack->Intercept.Z) * pTSTrack->Slope;
		pTSTrack->Sigma += T * T;
		};
...
bool TSReconstructionManager::RefineAlignment(void *pContext, ActionProgressCallBack ProgressMonitor, CString *pNotes)
{
