...
class SegCellArray : public CellArray<TS_Segment>
{
	public:
		...
		SegCellArray(int Count, TS_Segment *pItems, float ReferenceZ, float XTol, float YTol) 
			: CellArray<TS_Segment>(Count, pItems, ReferenceZ, XTol, YTol) 
		{};
...
void TSReconstructionManager::ClearTracks()
{
	if (pTSReconstruction)
	{
		...
		{
			if (pTSReconstruction->pLayers[i].pSegments[0])
			{
				if (pTSReconstruction->pLayers[i].pSegments[0][0].pTracks)
					free(pTSReconstruction->pLayers[i].pSegments[0][0].pTracks);
				free(pTSReconstruction->pLayers[i].pSegments[0]);
				};
			if (pTSReconstruction->pLayers[i].pRawTracks[0]) free(pTSReconstruction->pLayers[i].pRawTracks[0]);
			};
...
bool TSReconstructionManager::AddLayer(TS_LayerInfo *pLayer, bool bResetDepth, bool bAlignSlopes)
{
	...
	if (pLayer->RTCount[1] || pLayer->pRawTracks[1]) return false;
	...
	pTSReconstruction->pLayers[NextLayer].pRawTracks[0] = (TS_RawTrack *)malloc(sizeof(TS_RawTrack) * pLayer->RTCount[0]);
	memcpy(pTSReconstruction->pLayers[NextLayer].pRawTracks[0], pLayer->pRawTracks[0], sizeof(TS_RawTrack) * pLayer->RTCount[0]);
	pTSReconstruction->pLayers[NextLayer].pRawTracks[1] = 0;
	...
	try
	{
		...
		else
		{
			...
			for (i = 0; i < pTSRec->Hdr.SheetsCount; i++)
			{
				...
				if (pTSRec->pLayers[i].RTCount[1] || pTSRec->pLayers[i].SCount[1]) throw 0; 

				pTSReconstruction->pLayers[i].pRawTracks[0] = (TS_RawTrack *)malloc(sizeof(TS_RawTrack) * pTSRec->pLayers[i].RTCount[0]);
				memcpy(pTSReconstruction->pLayers[i].pRawTracks[0], pTSRec->pLayers[i].pRawTracks[0], sizeof(TS_RawTrack) * pTSRec->pLayers[i].RTCount[0]);
				pTSReconstruction->pLayers[i].pSegments[0] = (TS_Segment *)malloc(sizeof(TS_Segment) * pTSRec->pLayers[i].SCount[0]);
				memcpy(pTSReconstruction->pLayers[i].pSegments[0], pTSRec->pLayers[i].pSegments[0], sizeof(TS_Segment) * pTSRec->pLayers[i].SCount[0]);
				pTSReconstruction->pLayers[i].pSegments[0][0].pTracks = (TS_RawTrack **)malloc(sizeof(TS_RawTrack *) * pTSRec->pLayers[i].SCount[0] * pTSRec->Hdr.MaxTracksInSegment);
				for (j = 0; j < pTSRec->pLayers[i].SCount[0]; j++)
				{
					pTSReconstruction->pLayers[i].pSegments[0][j].pTracks = pTSReconstruction->pLayers[i].pSegments[0][0].pTracks + j * pTSRec->Hdr.MaxTracksInSegment;
					for (k = 0; k < pTSRec->Hdr.MaxTracksInSegment; k++)
						pTSReconstruction->pLayers[i].pSegments[0][j].pTracks[k] = (pTSRec->pLayers[i].pSegments[0][j].pTracks[k]) ? ((pTSRec->pLayers[i].pSegments[0][j].pTracks[k] - 
							pTSRec->pLayers[i].pRawTracks[0]) + pTSReconstruction->pLayers[i].pRawTracks[0]) : 0;
					};
				};
			for (i = 0; i < pTSRec->Hdr.SheetsCount; i++)
			{	
				for (j = 0; j < pTSRec->pLayers[i].SCount[0]; j++)
				{				
					if (pTSRec->pLayers[i].pSegments[0][j].pBottomLink)
					{
						for (k = 0; k < pTSRec->Hdr.SheetsCount; k++)
						{
							index = pTSRec->pLayers[i].pSegments[0][j].pBottomLink - pTSRec->pLayers[k].pSegments[0];
							if ((index >= 0) && (index < pTSRec->pLayers[k].SCount[0]))
							{
								pTSReconstruction->pLayers[i].pSegments[0][j].pBottomLink = index + pTSReconstruction->pLayers[k].pSegments[0];
								break;
								};
							};
						if (k >= pTSRec->Hdr.SheetsCount) throw 1;
						};
					if (pTSRec->pLayers[i].pSegments[0][j].pTopLink)
					{
						for (k = 0; k < pTSRec->Hdr.SheetsCount; k++)
						{
							index = pTSRec->pLayers[i].pSegments[0][j].pTopLink - pTSRec->pLayers[k].pSegments[0];
							if ((index >= 0) && (index < pTSRec->pLayers[k].SCount[0]))
							{
								pTSReconstruction->pLayers[i].pSegments[0][j].pTopLink = index + pTSReconstruction->pLayers[k].pSegments[0];
								break;
								};
							};
						if (k >= pTSRec->Hdr.SheetsCount) throw 1;
		...
		for (i = 0; i < pTSRec->Hdr.TSTracksCount; i++)
		{
			for (k = 0; k < pTSRec->Hdr.SheetsCount; k++)
			{
				index = pTSRec->pTracks[i].pBottomEnd - pTSRec->pLayers[k].pSegments[0];
				if ((index >= 0) && (index < pTSRec->pLayers[k].SCount[0]))
				{
					pTSReconstruction->pTracks[i].pBottomEnd = index + pTSReconstruction->pLayers[k].pSegments[0];
					break;
					};
				};
			if (k >= pTSRec->Hdr.SheetsCount) throw 2;
	
			for (k = 0; k < pTSRec->Hdr.SheetsCount; k++)
			{
				index = pTSRec->pTracks[i].pTopEnd - pTSRec->pLayers[k].pSegments[0];
				if ((index >= 0) && (index < pTSRec->pLayers[k].SCount[0]))
				{
					pTSReconstruction->pTracks[i].pTopEnd = index + pTSReconstruction->pLayers[k].pSegments[0];
					break;
					};
				};
			if (k >= pTSRec->Hdr.SheetsCount) throw 3;
...
void TSReconstructionManager::ResetDepth(TS_LayerInfo *pLayer)
{
	...
	for (i = 0; i < pLayer->RTCount[0]; i++)
	{
		pLayer->pRawTracks[0][i].Intercept.Z += DZ;
		pLayer->pRawTracks[0][i].FirstZ += DZ;
		pLayer->pRawTracks[0][i].LastZ += DZ;
		};
...
void TSReconstructionManager::AlignSlopes(TS_LayerInfo *pLayer)
{
	...
	for (i = 0; i < pLayer->RTCount[0]; i++)
	{
		pLayer->pRawTracks[0][i].Slope.X -= (pLayer->GlobalSideAlign[0].DSlope[0] + pLayer->pRawTracks[0][i].Slope.X * pLayer->GlobalSideAlign[0].DShrink[0]);
		pLayer->pRawTracks[0][i].Slope.Y -= (pLayer->GlobalSideAlign[0].DSlope[1] + pLayer->pRawTracks[0][i].Slope.Y * pLayer->GlobalSideAlign[0].DShrink[1]);
		};
	};

bool TSReconstructionManager::FormSegments(TS_LayerInfo *pLayer)
{
	int MaxTks = pTSReconstruction->Hdr.MaxTracksInSegment;
	if (MaxTks < 1) return false;
	int i1, i2, j1, j2, n = pLayer->RTCount[0];
	TS_RawTrack *qTracks = pLayer->pRawTracks[0];	
	if (pLayer->pSegments[0]) 
	{
		if (pLayer->pSegments[0][0].pTracks) free(pLayer->pSegments[0][0].pTracks);
		free(pLayer->pSegments[0]);
		pLayer->pSegments[0] = 0;		
		};
	pLayer->SCount[0] = 0;
	try
	{
		if (!n) return true;		
		TS_Segment *pSegs = pLayer->pSegments[0] = (TS_Segment *)malloc(n * sizeof(TS_Segment));
		...
		SegCellArray CA(n, pSegs, MinZ, 
			TSRD.LocalityCellSize + ((TSRD.InField_D_Pos > TSRD.OffField_D_Pos) ? TSRD.InField_D_Pos : TSRD.OffField_D_Pos) + 
...
bool TSReconstructionManager::LinkSegments(int BottomLayer, int TopLayer, bool OptimizeAlignment)
{
	int BottomSegCount = pTSReconstruction->pLayers[BottomLayer].SCount[0];
	int TopSegCount = pTSReconstruction->pLayers[TopLayer].SCount[0];
	if (BottomSegCount <= 0 || TopSegCount <= 0) return false;
	TS_Segment *pBottomSegArray = pTSReconstruction->pLayers[BottomLayer].pSegments[0];
	TS_Segment *pTopSegArray = pTSReconstruction->pLayers[TopLayer].pSegments[0];
	...
	{
		PosTol = TSRD.Gross_D_Pos;
		SlopeTol = TSRD.Gross_D_Slope;
		MaxIters = TSRD.MaxIters;
		}
	else 
	{
		PosTol = TSRD.D_Pos;
		SlopeTol = TSRD.D_Slope;
		MaxIters = 1;		
		};
	...
	if (OptimizeAlignment)
	{		
		if (TSRD.PrescanMode == TSRD_PRESCAN_NONE)
		{
			Align[0] = 0.;
			Align[1] = 0.;
			Align[2] = 0.;
		...
		else
		{
			...
			Vector2 *MapPattern = 0;

			Vector2 MapPattern1[1] =
			{
				Vector2(0., 0.)
				};

			Vector2 MapPattern4[4] =
			{
				Vector2(-0.5, -0.5), Vector2(0.5, -0.5), Vector2(-0.5, 0.5), Vector2(0.5, 0.5)
				};

			if (TSRD.PrescanMode == TSRD_PRESCAN_TRANSLATION)
			{
				MapPatternChances = 1;
			...
			Vector2 TempMapPos[4];
			Vector2 TempMapDelta[4];
			int TempMapCount[4];
			Vector2 MapPos[3];
			Vector2 MapDelta[3];
			int trial, succ;
			float RefZ = 0.5f * (pTSReconstruction->pLayers[BottomLayer].RefDepth + pTSReconstruction->pLayers[TopLayer].RefDepth);
			for (trial = succ = 0; (trial < MapPatternChances); trial++)
			{
				...
				int i, botcount, topcount;
				TempMapPos[trial].X = MapPattern[trial].X * TSRD.PrescanLeverArm;
				TempMapPos[trial].Y = MapPattern[trial].Y * TSRD.PrescanLeverArm;
				float MinZoneX, MinZoneY, MaxZoneX, MaxZoneY;
				float MinExtX, MinExtY, MaxExtX, MaxExtY;
				MinZoneX = TempMapPos[trial].X + pTSReconstruction->Hdr.RefCenter.X - TSRD.PrescanZoneWidth * 0.5;
				MaxZoneX = TempMapPos[trial].X + pTSReconstruction->Hdr.RefCenter.X + TSRD.PrescanZoneWidth * 0.5;
				MinZoneY = TempMapPos[trial].Y + pTSReconstruction->Hdr.RefCenter.Y - TSRD.PrescanZoneWidth * 0.5;
				MaxZoneY = TempMapPos[trial].Y + pTSReconstruction->Hdr.RefCenter.Y + TSRD.PrescanZoneWidth * 0.5;
				MinExtX = TempMapPos[trial].X + pTSReconstruction->Hdr.RefCenter.X - TSRD.PrescanExtents * 0.5;
				MaxExtX = TempMapPos[trial].X + pTSReconstruction->Hdr.RefCenter.X + TSRD.PrescanExtents * 0.5;
				MinExtY = TempMapPos[trial].Y + pTSReconstruction->Hdr.RefCenter.Y - TSRD.PrescanExtents * 0.5;
				MaxExtY = TempMapPos[trial].Y + pTSReconstruction->Hdr.RefCenter.Y + TSRD.PrescanExtents * 0.5;
				for (i = botcount = 0; i < BottomSegCount; i++)
				{
					pBottomProj[botcount] = pBottomSegArray[i];
					Vector T = pBottomSegArray[i].Intercept + (RefZ - pBottomSegArray[i].Intercept.Z) * pBottomSegArray[i].Slope;
					if (T.X >= MinZoneX && T.X <= MaxZoneX && T.Y >= MinZoneY && T.Y <= MaxZoneY)			
					{
						pBottomProj[botcount].Intercept.X = T.X;
						pBottomProj[botcount].Intercept.Y = T.Y;
						pBottomProj[botcount].Intercept.Z = T.Z;
						botcount++;
						};
					};
				for (i = topcount = 0; i < TopSegCount; i++)
				{
					pTopProj[topcount] = pTopSegArray[i];
					Vector T = pTopSegArray[i].Intercept + (RefZ - pTopSegArray[i].Intercept.Z) * pTopSegArray[i].Slope;
					if (T.X >= MinExtX && T.X <= MaxExtX && T.Y >= MinExtY && T.Y <= MaxExtY)
					{
						pTopProj[topcount].Intercept.X = T.X;
						pTopProj[topcount].Intercept.Y = T.Y;
						pTopProj[topcount].Intercept.Z = T.Z;
						topcount++;
						};
					};
				...
				TempMapDelta[trial] = StartParams.RawTransl;
				TempMapCount[trial] = StartParams.CoincN;
				if (StartParams.CoincN > 0 /* ??? */)
				{
					succ++;
					};
				};
			if (succ < ((TSRD.PrescanMode == TSRD_PRESCAN_TRANSLATION) ? 1 : 3)) return false;
			{
				int index, compindex, count;
...
//		StartParams.RawTransl.X = StartParams.RawTransl.Y = 0.f;
	...
	CellArray<TS_Segment> CA(BottomSegCount, pBottomSegArray, MinZ, ((TSRD.Gross_D_Pos > TSRD.D_Pos) ? TSRD.Gross_D_Pos : TSRD.D_Pos) + ((TSRD.Gross_D_Slope > TSRD.D_Slope) ? TSRD.Gross_D_Slope : TSRD.D_Slope) * (MaxZ - MinZ),
		((TSRD.Gross_D_Pos > TSRD.D_Pos) ? TSRD.Gross_D_Pos : TSRD.D_Pos) + ((TSRD.Gross_D_Slope > TSRD.D_Slope) ? TSRD.Gross_D_Slope : TSRD.D_Slope) * (MaxZ - MinZ) + TSRD.LocalityCellSize);

	for (Iteration = 0; Iteration < MaxIters; Iteration++)
	{
		PosTol = ((float)(Iteration + 1) * TSRD.D_Pos + (MaxIters - Iteration - 1) * TSRD.Gross_D_Pos) / MaxIters;
		SlopeTol = ((float)(Iteration + 1) * TSRD.D_Slope + (MaxIters - Iteration - 1) * TSRD.Gross_D_Slope) / MaxIters;
		int i1, i2;
		for (i1 = 0; i1 < BottomSegCount; i1++)	pBottomLinkQuality[i1] = 1. + TSRD.Gross_D_Pos + TSRD.D_Pos + TSRD.Gross_D_Slope + TSRD.D_Slope;
		for (i2 = 0; i2 < TopSegCount; i2++) pTopLinkQuality[i2] = 1. + TSRD.Gross_D_Pos + TSRD.D_Pos + TSRD.Gross_D_Slope + TSRD.D_Slope;
		int count = 0;
			...
			for (locki1 = 0; locki1 < CA.ExternalLock.Count; locki1++)
			{
					...
					S1.Slope.Y - (S2.Slope.Y + SAlignDSlope.Y + S2.Slope.Y * SAlignDShrink.Y)) <= SlopeTol)
				{
					...
					if (Dist <= PosTol)
					{
						...
						{	
							if (pBottomLinkQuality[i1] < Dist || pTopLinkQuality[i2] < Dist) continue;
							{
								...
								if (S1.pTopLink)
								{
									...
									pTopLinkQuality[index2] = pBottomLinkQuality[index1] = 1. + TSRD.Gross_D_Pos + TSRD.D_Pos + TSRD.Gross_D_Slope + TSRD.D_Slope;
									count--;
									};
								if (S2.pBottomLink)
								{
									...
									pBottomLinkQuality[index1] = pTopLinkQuality[index2] = 1. + TSRD.Gross_D_Pos + TSRD.D_Pos + TSRD.Gross_D_Slope + TSRD.D_Slope;
									count--;
									};
								};
						...
						count++;
						if (OptimizeAlignment && (hypot(S1.Slope.X - TSRD.AlignBeamXSlope, S1.Slope.Y - TSRD.AlignBeamYSlope) < TSRD.AlignBeamWidth) && (!TSRD.AlignOnLinked || ((BottomLayer == pTSReconstruction->Hdr.SheetsCount - 1) || S1.pBottomLink)))
						{							
							...
							SlS.X += S1.Slope.X;
							SlS.Y += S1.Slope.Y;
							SlS2.X += S1.Slope.X * S1.Slope.X;
							SlS2.Y += S1.Slope.Y * S1.Slope.Y;
							SlSD.X += S1.Slope.X * DS.X;
							SlSD.Y += S1.Slope.Y * DS.Y;
							};
						};
					};
				};
	...
	if (OptimizeAlignment)
	{
		pAlignLayer->LocalSideAlign[0].PosAlign.M[0][0] = Align[0];
		pAlignLayer->LocalSideAlign[0].PosAlign.M[0][1] = Align[1];
		pAlignLayer->LocalSideAlign[0].PosAlign.M[1][0] = Align[2];
		pAlignLayer->LocalSideAlign[0].PosAlign.M[1][1] = Align[3];
		pAlignLayer->LocalSideAlign[0].PosAlign.D[0] = Align[4];
		pAlignLayer->LocalSideAlign[0].PosAlign.D[1] = Align[5];
		pAlignLayer->LocalSideAlign[0].PosAlign.D[2] = Align[6];
		pAlignLayer->LocalSideAlign[0].SlopeAlign.DShrink[0] = SAlignDShrink.X;
		pAlignLayer->LocalSideAlign[0].SlopeAlign.DShrink[1] = SAlignDShrink.Y;
		pAlignLayer->LocalSideAlign[0].SlopeAlign.DSlope[0] = SAlignDSlope.X;
		pAlignLayer->LocalSideAlign[0].SlopeAlign.DSlope[1] = SAlignDSlope.Y;
		};
	if (pBottomLinkQuality) free(pBottomLinkQuality);
	if (pTopLinkQuality) free(pTopLinkQuality);
	return true;
...
bool TSReconstructionManager::Reconstruct(void *pContext, TSReconstructionManager::ActionProgressCallBack ProgressMonitor, CString *pNotes)
{
	...
	for (i = 0; i < pTSReconstruction->Hdr.SheetsCount; i++)
	{
		FormSegments(pTSReconstruction->pLayers + i);
		if (ProgressMonitor) ProgressMonitor(pContext, "Creating Segments", (float)i / (float)pTSReconstruction->Hdr.SheetsCount);
		};

	Vector RefCenter(0.f, 0.f, 0.f); 
	for (i = t = 0; i < pTSReconstruction->Hdr.SheetsCount; i++)
	{
		t += pTSReconstruction->pLayers[i].SCount[0];
		for (j = 0; j < pTSReconstruction->pLayers[i].SCount[0]; j++)
		{
			RefCenter = RefCenter + pTSReconstruction->pLayers[i].pSegments[0][j].Intercept + 
				(0.5f * (pTSReconstruction->pLayers[i].pSegments[0][j].FirstZ + pTSReconstruction->pLayers[i].pSegments[0][j].LastZ) - 
				pTSReconstruction->pLayers[i].pSegments[0][j].Intercept.Z) * pTSReconstruction->pLayers[i].pSegments[0][j].Slope;
			};
		};
	if (t)
	{
	...
	TotalCycles = 0.5 * ((pTSReconstruction->Hdr.SheetsCount - 1) * (pTSReconstruction->Hdr.SheetsCount) - (pTSReconstruction->Hdr.SheetsCount - TSRD.MaxMissingSegments) * (pTSReconstruction->Hdr.SheetsCount - TSRD.MaxMissingSegments - 1));
	Cycle = 0;
	...
	for ((i = pTSReconstruction->Hdr.SheetsCount - 2), (j = pTSReconstruction->Hdr.SheetsCount - 1); i >= 0; i--)
	{
		if (LinkSegments(j, i, true))
		{			
			TS_Segment *pSegs = pTSReconstruction->pLayers[i].pSegments[0];
			int SegsCount = pTSReconstruction->pLayers[i].SCount[0];

			for (k = 0; k < SegsCount; k++)
			{
				pSegs[k].Slope.X = (1. + pTSReconstruction->pLayers[i].LocalSideAlign[0].SlopeAlign.DShrink[0]) * pSegs[k].Slope.X + pTSReconstruction->pLayers[i].LocalSideAlign[0].SlopeAlign.DSlope[0];
				pSegs[k].Slope.Y = (1. + pTSReconstruction->pLayers[i].LocalSideAlign[0].SlopeAlign.DShrink[1]) * pSegs[k].Slope.Y + pTSReconstruction->pLayers[i].LocalSideAlign[0].SlopeAlign.DSlope[1];
				};
			pTSReconstruction->pLayers[i].LocalSideAlign[0].PosAlign.Reserved[0] = 0;
			j = i;
			}
		else 
		{
			pTSReconstruction->pLayers[i].LocalSideAlign[0].PosAlign.Reserved[0] = 1;
			if (pNotes) 
			{
				CString Temp;
				Temp.Format("Failed first-pass linking Layer %d Sheet %d and Layer %d Sheet %d.\r\n", 
					i, pTSReconstruction->pLayers[i].SheetId, j, pTSReconstruction->pLayers[j].SheetId);
				*pNotes += Temp;
				};
			};
	...
	for (j = pTSReconstruction->Hdr.SheetsCount - 2; j >= 0; j--)
	{		
		if (pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.Reserved[0] > 0) continue;
		Vector OD = D;
		double OM[2][2] = { {M[0][0], M[0][1]}, {M[1][0], M[1][1]} };					
		double TM[2][2] = 
		{ 
			{1. + pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[0][0], pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[0][1]},
			{pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[1][0], 1. + pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[1][1]}
			};
		D.Z = OD.Z + pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.D[2];
		D.X = TM[0][0] * OD.X + TM[0][1] * OD.Y + pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.D[0];
		D.Y = TM[1][0] * OD.X + TM[1][1] * OD.Y + pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.D[1];
		...
		pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[0][0] = M[0][0];
		pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[0][1] = M[0][1];
		pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[1][0] = M[1][0];
		pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[1][1] = M[1][1];
		pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.D[0] = D.X;
		pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.D[1] = D.Y;
		pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.D[2] = D.Z;
		};
...
*/
	...
	for (j = pTSReconstruction->Hdr.SheetsCount - 1; j >= 0; j--)
	{
		if (pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.Reserved[0] > 0) continue;
		TS_Segment *pSegs = pTSReconstruction->pLayers[j].pSegments[0];
		int SegsCount = pTSReconstruction->pLayers[j].SCount[0];
		double RefDZ = pTSReconstruction->pLayers[j].RelevantZs.TopInt - pTSReconstruction->pLayers[pTSReconstruction->Hdr.SheetsCount - 1].RelevantZs.TopInt;
		D.X = (pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.D[0] -= GlobalShift.X * RefDZ);
		D.Y = (pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.D[1] -= GlobalShift.Y * RefDZ);
		D.Z = (pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.D[2] -= GlobalShift.Z * RefDZ);
		M[0][0] = pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[0][0];
		M[0][1] = pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[0][1];
		M[1][0] = pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[1][0];
		M[1][1] = pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.M[1][1];

		pTSReconstruction->pLayers[j].LocalSideAlign[0].SlopeAlign.DShrink[0] *= (1. - GlobalShift.Z);
		pTSReconstruction->pLayers[j].LocalSideAlign[0].SlopeAlign.DShrink[1] *= (1. - GlobalShift.Z);
		pTSReconstruction->pLayers[j].LocalSideAlign[0].SlopeAlign.DSlope[0] = 
			(1. - GlobalShift.Z) * pTSReconstruction->pLayers[j].LocalSideAlign[0].SlopeAlign.DSlope[0] - GlobalShift.X;
		pTSReconstruction->pLayers[j].LocalSideAlign[0].SlopeAlign.DSlope[1] = 
			(1. - GlobalShift.Z) * pTSReconstruction->pLayers[j].LocalSideAlign[0].SlopeAlign.DSlope[1] - GlobalShift.Y;
	...
	for (s = 1; s <= TSRD.MaxMissingSegments; s++)
		for (i = pTSReconstruction->Hdr.SheetsCount - 1; i > s; i--)
		{
			j = i - s - 1;
			if (pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.Reserved[0] > 0) 
			{
				if (pNotes) 
				{
					CString Temp;
					Temp.Format("Skipped second-pass linking Layer %d Sheet %d and Layer %d Sheet %d.\r\n", 
						i, pTSReconstruction->pLayers[i].SheetId, j, pTSReconstruction->pLayers[j].SheetId);
					*pNotes += Temp;
					};
				}
			else if (!LinkSegments(i, j, false)) 
			{
				if (pNotes) 
				{
					CString Temp;
					Temp.Format("Failed second-pass linking Layer %d Sheet %d and Layer %d Sheet %d.\r\n", 
						i, pTSReconstruction->pLayers[i].SheetId, j, pTSReconstruction->pLayers[j].SheetId);
					*pNotes += Temp;
					};
				return false;
		...
		{		
			...
			for (j = 0; j < SegsCount; j++)
				if (!pSegs[j].pTopLink)
				{
					...
					if ((Temp.Count >= TSRD.MinSegments) && FitTrack(&Temp))
					{
			...
			if (ProgressMonitor) ProgressMonitor(pContext, "Building Alignment Tracks", (float)(i + 1) / (float)(pTSReconstruction->Hdr.SheetsCount));
			};
...
*/
			for (i = 0; i < pTSReconstruction->Hdr.SheetsCount; i++)
			{
				double RefDZ = pTSReconstruction->pLayers[i].RelevantZs.TopInt - pTSReconstruction->pLayers[pTSReconstruction->Hdr.SheetsCount - 1].RelevantZs.TopInt;
				TS_LayerInfo::t_LocalSideAlign &a = pTSReconstruction->pLayers[i].LocalSideAlign[2 + 0];
				...
				{
					TVector I, S;
					TS_Segment *pSeg = pTSReconstruction->pLayers[i].pSegments[0] + j;
			...
			for (s = 0; s <= TSRD.MaxMissingSegments; s++)
				for (i = pTSReconstruction->Hdr.SheetsCount - 1; i > s; i--)
				{
					...
					if (pTSReconstruction->pLayers[j].LocalSideAlign[0].PosAlign.Reserved[0] > 0)
					{
						...
						{
							CString Temp;
							Temp.Format("Skipped relinking Layer %d Sheet %d and Layer %d Sheet %d.\r\n", 
								i, pTSReconstruction->pLayers[i].SheetId, j, pTSReconstruction->pLayers[j].SheetId);
							*pNotes += Temp;
							};
						}
					else if (!LinkSegments(i, j, false)) 
					{
						...
						{
							CString Temp;
							Temp.Format("Failed relinking Layer %d Sheet %d and Layer %d Sheet %d.\r\n", 
								i, pTSReconstruction->pLayers[i].SheetId, j, pTSReconstruction->pLayers[j].SheetId);
							*pNotes += Temp;
							};
						return false;
...
bool TSReconstructionManager::CheckTopology(void *pContext, TSReconstructionManager::ActionProgressCallBack ProgressMonitor, CString *pNotes)
{
	...
	{		
		...
		for (j = 0; j < SegsCount; j++)
			if (!pSegs[j].pTopLink)
			{
				...
				bool Good;
				if (((Good = (Temp.Count >= TSRD.MinSegments)) || 
		...
		if (ProgressMonitor) ProgressMonitor(pContext, "Building Tracks", (float)(i + 1) / (float)(pTSReconstruction->Hdr.SheetsCount));
		};
	...
	for (i = 0; i < pTSReconstruction->Hdr.SheetsCount; i++)
		for (j = 0; j < pTSReconstruction->pLayers[i].SCount[0]; j++)
		{
			TS_Segment Seg = pTSReconstruction->pLayers[i].pSegments[0][j];
...
{	
	...
	{		
		SegPtrToIndices(pSeg, &i);
		Z = pTSReconstruction->pLayers[i].RelevantZs.TopInt - R.Z;
		Sz += Z;
		Sz2 += Z * Z;
		};
	Den = Sz * Sz - (j * Sz2);
	...
	{		
		SegPtrToIndices(pSeg, &i);
		Vector T, S = pSeg->Slope, P = pSeg->Intercept;
		Z = pTSReconstruction->pLayers[i].RelevantZs.TopInt;
...
bool TSReconstructionManager::SegPtrToIndices(TS_Segment *pSeg, int *i)
{
	int j, k;
	for (j = 0; j < pTSReconstruction->Hdr.SheetsCount; j++)
	{
		k = pSeg - pTSReconstruction->pLayers[j].pSegments[0];
		if (k >= 0 && k < pTSReconstruction->pLayers[j].SCount[0]) 
		{
			*i = j;
			return true;
			};
...
#ifdef OLD_REALIGN
		SegPtrToIndices(pSeg, &i);
		TS_LayerInfo::t_LocalSideAlign &a = pTSReconstruction->pLayers[i].LocalSideAlign[2];
...
bool TSReconstructionManager::RefineAlignment(void *pContext, TSReconstructionManager::ActionProgressCallBack ProgressMonitor, CString *pNotes)
{
	...
	for (i = savesegcount = 0; i < pTSReconstruction->Hdr.TSTracksCount; savesegcount += pTSReconstruction->pTracks[i++].Count);
	pSaveSegs = (SaveSeg *)malloc(sizeof(SaveSeg) * savesegcount);
	for (i = savesegcount = 0; i < pTSReconstruction->Hdr.TSTracksCount; i++)
	{
		TS_Segment *pScan = pTSReconstruction->pTracks[i].pTopEnd;
		while (pScan)
		{
			pSaveSegs[savesegcount].pSeg = pScan;
			pSaveSegs[savesegcount].Save = *pScan;
			savesegcount++;
			pScan = pScan->pBottomLink;
			};
		};
	...
	for (h = 0; h < TSRD.FinalAlignment_MaxIters; h++)
	{
		...
		for (i = 0; i < pTSReconstruction->Hdr.TSTracksCount; i++)
			if (pTSReconstruction->pTracks[i].Count >= TSRD.FinalAlignment_SlopeMeasures)
			{
				...
				for ((pStart = pTSReconstruction->pTracks[i].pBottomEnd), (j = 0); 
					...
					if (LocalFit(pStart, TSRD.FinalAlignment_SlopeMeasures, &LocalSlope) && (hypot(LocalSlope.X - TSRD.AlignBeamXSlope, LocalSlope.Y - TSRD.AlignBeamYSlope) < TSRD.AlignBeamWidth))
					{
						count += TSRD.FinalAlignment_SlopeMeasures;
						for ((k = 0), (pSeg = pStart); k < TSRD.FinalAlignment_SlopeMeasures; (pSeg = pSeg->pTopLink), k++)
						{
#ifdef OLD_REALIGN
							SegPtrToIndices(pSeg, &ql);
							TS_LayerInfo::t_LocalSideAlign &a = pTSReconstruction->pLayers[ql].LocalSideAlign[2];
...
#else							
							AngleDelta += (pSeg->Slope.X - LocalSlope.X) * (pSeg->Slope.X - LocalSlope.X) + (pSeg->Slope.Y - LocalSlope.Y) * (pSeg->Slope.Y - LocalSlope.Y);
#endif
							};
						};
		...
		if (count < 1)
		{
			...
			for (i = 0; i < savesegcount; i++) 
				*(pSaveSegs[i].pSeg) = pSaveSegs[i].Save; 
			if (pSaveSegs) free(pSaveSegs);
			return false;
			};																
		...
		for (i = good = 0; i < pTSReconstruction->Hdr.TSTracksCount; i++)
		{
			if (pTSReconstruction->pTracks[i].Count >= TSRD.FinalAlignment_SlopeMeasures)
			{
				...
				Vector2 T, U;
				if (LocalFit(pTSReconstruction->pTracks[i].pBottomEnd, TSRD.FinalAlignment_SlopeMeasures, &T) && (hypot(T.X - TSRD.AlignBeamXSlope, T.Y - TSRD.AlignBeamYSlope) < TSRD.AlignBeamWidth) &&
					LocalFit(pExit, TSRD.FinalAlignment_SlopeMeasures, &U) && (hypot(U.X - TSRD.AlignBeamXSlope, U.Y - TSRD.AlignBeamYSlope) < TSRD.AlignBeamWidth))
		...
		for (i = 0; i < good; i++)
		{			
			SegPtrToIndices(ppBottomSegs[i] = ppTracks[i]->pBottomEnd /*pTSReconstruction->pTracks[i].pBottomEnd*/, &j);
			pBottomSegLayerIndices[i] = j;
			SegPtrToIndices(ppTopSegs[i] = ppBottomSegs[i]->pTopLink, &j);
			pTopSegLayerIndices[i] = j;
			};

#ifndef OLD_REALIGN
		for (i = 0; i < savesegcount; i++) *(pSaveSegs[i].pSeg) = pSaveSegs[i].Save;
#endif

		for (k = pTSReconstruction->Hdr.SheetsCount - 2; k >= 0; k--)
		{
			TS_LayerInfo::t_LocalSideAlign &rb = pTSReconstruction->pLayers[k].LocalSideAlign[2];
			count = 0;
			...
			for (i = 0; i < good; i++)
			{
				if (ppTopSegs[i] && pTopSegLayerIndices[i] == k)
				{
					...
					TS_LayerInfo::t_LocalSideAlign &ra = pTSReconstruction->pLayers[pBottomSegLayerIndices[i]].LocalSideAlign[2];
#ifdef OLD_REALIGN
					...
					Vector2 D(ra.PosAlign.M[0][0] * (pBottom->Intercept.X - RefCenter.X) + ra.PosAlign.M[0][1] * (pBottom->Intercept.Y - RefCenter.Y) + 
						...
						(pTop->Intercept.X - RefCenter.X) - (RefZ - pTop->Intercept.Z) * ((1. + rb.SlopeAlign.DShrink[0]) * S2.X + rb.SlopeAlign.DSlope[0]),
						...
						(pTop->Intercept.Y - RefCenter.Y) - (RefZ - pTop->Intercept.Z) * ((1. + rb.SlopeAlign.DShrink[1]) * S2.Y + rb.SlopeAlign.DSlope[1]));
					...
					SlSD.Y += S2.Y * DS.Y;
#else
					double RefZ = 0.5 * (pBottom->Intercept.Z + pTop->Intercept.Z);
					Vector2 P(pBottom->Intercept.X - RefCenter.X, pBottom->Intercept.Y - RefCenter.Y);
					Vector2 S(pBottom->Slope);
					Vector2 S2(pTop->Slope);
					Vector2 D(P.X + (RefZ - pBottom->Intercept.Z) * S.X - 
						(pTop->Intercept.X - RefCenter.X) - (RefZ - pTop->Intercept.Z) * (S2.X + rb.SlopeAlign.DShrink[0] * S.X + rb.SlopeAlign.DSlope[0]),
						P.Y + (RefZ - pBottom->Intercept.Z) * S.Y - 
						(pTop->Intercept.Y - RefCenter.Y) - (RefZ - pTop->Intercept.Z) * (S2.Y + rb.SlopeAlign.DShrink[1] * S.Y + rb.SlopeAlign.DSlope[1]));
			...
			if (count < TSRD.FinalAlignment_MinEntries || count < POS_ALIGN_DATA_LEN)
			{
				...
				{
					CString Temp;
					Temp.Format("Skipped refinement on layer %d.\r\n", k);
					*pNotes += Temp;
					};
				rb = pTSReconstruction->pLayers[k - 1].LocalSideAlign[2];
				}
			else
			{
				LULinSystem L(POS_ALIGN_DATA_LEN, (double *)(void *)AlignMat);
...
//				SAlignDSlope.X = SlD.X / count;
//				SAlignDSlope.Y = SlD.Y / count;
				...
				for (i = 0; i < good; i++)
				{
					TS_Segment *pSeg = ppBottomSegs[i];
					if (pSeg && (pBottomSegLayerIndices[i] == k))
					{
						TS_Segment *pCorr = pSeg->pBottomLink;
						if (pCorr)
						{
							pSeg->Slope.X += SAlignDShrink.X * pCorr->Slope.X + SAlignDSlope.X;
							pSeg->Slope.Y += SAlignDShrink.Y * pCorr->Slope.Y + SAlignDSlope.Y;
							pSeg->Intercept.X += (rb.PosAlign.M[0][0] - 1.) * (pCorr->Intercept.X - RefCenter.X) + rb.PosAlign.M[0][1] * (pCorr->Intercept.Y - RefCenter.Y) + rb.PosAlign.D[0];
							pSeg->Intercept.Y += rb.PosAlign.M[1][0] * (pCorr->Intercept.X - RefCenter.X) + (rb.PosAlign.M[1][1] - 1.) * (pCorr->Intercept.Y - RefCenter.Y) + rb.PosAlign.D[1];
							pSeg->Intercept.Z -= rb.PosAlign.D[2];
							};
						};
					};
#endif
