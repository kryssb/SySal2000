...
#include "ThetaTSReconstruct_i.c"
#include "ThetaTSRecon.h"

CComModule _Module;

BEGIN_OBJECT_MAP(ObjectMap)
OBJECT_ENTRY(CLSID_ThetaTSReconstruct, CTSRecon)
END_OBJECT_MAP()
...
{
    _Module.Init(ObjectMap, m_hInstance, &LIBID_THETATSRECONSTRUCTLib);
...
STDAPI DllRegisterServer(void)
{
	...
	try
	{
		...
		MyKey.Close();
		if (MyKey.Create(HKEY_LOCAL_MACHINE, "Software\\SySal2\\Classes\\ThetaTSReconstruct", REG_NONE, REG_OPTION_NON_VOLATILE, KEY_WRITE | KEY_READ) != ERROR_SUCCESS) throw 3;
		MyKey.Close();
		MyKey.Open(HKEY_LOCAL_MACHINE, "Software\\SySal2\\Classes\\ThetaTSReconstruct", KEY_WRITE | KEY_READ);
		char CLSIDString[64];
		wsprintf(CLSIDString, "{%08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X}", 
			CLSID_ThetaTSReconstruct.Data1, CLSID_ThetaTSReconstruct.Data2, CLSID_ThetaTSReconstruct.Data3, 
			CLSID_ThetaTSReconstruct.Data4[0], CLSID_ThetaTSReconstruct.Data4[1], CLSID_ThetaTSReconstruct.Data4[2], CLSID_ThetaTSReconstruct.Data4[3], 
			CLSID_ThetaTSReconstruct.Data4[4], CLSID_ThetaTSReconstruct.Data4[5], CLSID_ThetaTSReconstruct.Data4[6], CLSID_ThetaTSReconstruct.Data4[7]);
	...
	catch (...)
	{
		...
		MyKey.RecurseDeleteKey("Software\\SySal2\\Classes\\ThetaTSReconstruct");
		MyKey.Close();
		};
