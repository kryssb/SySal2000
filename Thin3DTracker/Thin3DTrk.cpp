...
STDMETHODIMP CThin3DTracker::GetTracks(Track *pSpace, int *pMaxNumber, int StartLayer, int EndLayer)
{
	...
	IntGrain *PGrain, *PGrainEnd;
	IntGrain *PG1, *PGE1, *PG2, *PGE2, *PG3, *PGE3;
	Cell *PLayCell1, *PLayCell2, *PLayCell3, *PLayCell;
	Vector Start;
	Vector Slope;
	Vector Expect;
	IntGrain TempG1, TempG3;
	Track TempTrack;
	int i,j;
	int MDY, MDX;
	Track *SameTrack;
	...
	int CellsInLayer = T.CellsInLayer;
	Cell *CellSpace = T.CellSpace;
	IntGrain *GrainSpace = T.GrainSpace;
	...
	IntGrain **TempGrain = (IntGrain **)malloc(sizeof(IntGrain *) * T.LayerNum);
...
IntGrain *CThin3DTracker::NextGrain(Cell *PCell, IntGrain **TempGrain, Vector Expect)
{	IntGrain *PGrain, *PGrainEnd, *BestGrain = 0;
...
int CThin3DTracker::MergeTracks(Track *Id, int LastLay, int Npts, Track *Tracks, int NTracks,
	IntGrain **TempGrain, int MaxTracks)
{	int Lay, k;
...
HRESULT CThin3DTracker::GetFlatTracks(FlatTrack *pSpace, int *pMaxNumber, int MaxStripsInTrack, int StartLayer, int EndLayer)
{
	*pMaxNumber = 0;
	return E_NOTIMPL;
	};

HRESULT CThin3DTracker::GetFlatStrips(int Layer, FlatStrip *pSpace, int *pMaxNumber, float *pZ)
{
	*pMaxNumber = 0;
	return E_NOTIMPL;
	};

HRESULT CThin3DTracker::GetTracks2(Track2 *pSpace, int *pMaxNumber, int StartLayer, int EndLayer)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState())
													...
													{	
														...
														{	TempAbs1 = fabs(PG2->Xcm - Expect.X);
															...
															{	for (Lay = 0; Lay < Layer; Lay++) TempGrain[Lay] = 0;
																...
																if ((Npts >= (NptMin0 - !Vector(Slope.X, Slope.Y, 0.) * NptMinSlope)) && (NTracks < MaxTracks))
																{	TempTrack.Grains = Npts;
																	TempTrack.AreaSum = 0;
																	TempTrack.pGrains = pSpace[NTracks].pGrains;
																	for (i = 0, j = 0; i < Layer; i++)
																	{	if (PGrain = TempGrain[i])
																		{	TempTrack.pGrains[j].X = PGrain->Xcm;
																			TempTrack.pGrains[j].Y = PGrain->Ycm;
																			TempTrack.pGrains[j].Z = T.ZLayer[i];
																			TempTrack.pGrains[j].Area = PGrain->Area;
																			TempTrack.AreaSum += PGrain->Area;
																			j++;
#ifdef USED_GRAIN_CHECKING
																			PGrain->Used = pSpace + NTracks;
#endif
																	...
																	pSpace[NTracks].AreaSum = TempTrack.AreaSum;
																	pSpace[NTracks].Grains = TempTrack.Grains;
																	pSpace[NTracks].Valid = true;
																	NTracks++;
																	StopSearch = 1;
																	};
...
int CThin3DTracker::MergeTracks2(Track2 *Id, int LastLay, int Npts, Track2 *Tracks, int NTracks,
	IntGrain **TempGrain, int MaxTracks)
{	int Lay, k;
	float ZCheck = T.ZLayer[LastLay];
	Track2 Temp = *Id;
	...
	for (k = Temp.Grains - 1; (k > 0) && (ZCheck >= Temp.pGrains[k].Z); k--);

	if (k <= Temp.Grains - 4)
	{	if (NTracks < MaxTracks)
		{	Temp.Grains = k;
			Temp.pGrains = Tracks[NTracks].pGrains;
			for (Lay = 0; Lay < k; Lay++) Temp.pGrains[Lay] = Id->pGrains[Lay];
			for (Lay = k; Lay < Layer; Lay++)
			{	if (TempGrain[Lay])
				{	Temp.pGrains[Temp.Grains].X = TempGrain[Lay]->Xcm;
					Temp.pGrains[Temp.Grains].Y = TempGrain[Lay]->Ycm;
					Temp.pGrains[Temp.Grains].Z = T.ZLayer[Lay];
					Temp.AreaSum += TempGrain[Lay]->Area;
					Temp.Grains++;
#ifdef USED_GRAIN_CHECKING
					TempGrain[Lay]->Used = Tracks + NTracks;
#endif
			...
			Tracks[NTracks].Valid = true;
			Tracks[NTracks].AreaSum = Temp.AreaSum;
			Tracks[NTracks++].Grains = Temp.Grains;
			};
		}
	else
	{
		...
		Id->Grains = Temp.Grains;
		Id->AreaSum = Temp.AreaSum;
		};
	return NTracks;
	};
