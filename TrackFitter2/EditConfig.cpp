...
void CEditConfig::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EC_DISTWINDOWX, C.DistWinX);
	DDV_MinMaxFloat(pDX, C.DistWinX, 0.f, 50.f);
	DDX_Text(pDX, IDC_EC_DISTWINDOWY, C.DistWinY);
	DDV_MinMaxFloat(pDX, C.DistWinY, 0.f, 50.f);
	DDX_Check(pDX, IDC_EC_ENABLEPARABOLICCORRECTION, C.CorrectDistortion);
	DDX_Check(pDX, IDC_EC_ENABLERECOVERY, C.RecoveryOn);
	DDX_Text(pDX, IDC_EC_GOODPOINTS, C.GoodTrackMinPointsN);
	DDV_MinMaxUInt(pDX, C.GoodTrackMinPointsN, 3, 100);
	DDX_Text(pDX, IDC_EC_MAXDISTX, C.MaxDistX);
	DDV_MinMaxFloat(pDX, C.MaxDistX, 1.f, 100.f);
	DDX_Text(pDX, IDC_EC_MAXDISTY, C.MaxDistY);
	DDV_MinMaxFloat(pDX, C.MaxDistY, 1.f, 100.f);
	DDX_Text(pDX, IDC_EC_MAXITERATIONS, C.MaxIterations);
	DDV_MinMaxUInt(pDX, C.MaxIterations, 1, 30);
