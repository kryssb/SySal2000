...
#define make_x_alloc(x) ((x << 16) + mballoc)
	...
	try
	{
		...
		*/
		...
		if (!(pCSpace = (Cluster *)debug_GlobalAlloc(GMEM_FIXED, make_mballoc(sizeof(Cluster) * C.MaxClusters + sizeof(t_ProcessVariables::t_Rec) * ProcessVariables.FocusSamples)))) throw make_x_alloc(1);
