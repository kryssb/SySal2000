...
#define MYDEBUG(a) { FILE *f = fopen("d:\\kryss\\debug.txt", "a+t"); fprintf(f, "%s\n", a); fclose(f); }
#define MYDEBUGHEX(a, b) { FILE *f = fopen("d:\\kryss\\debug.txt", "a+t"); fprintf(f, "%s %08X\n", a, b); fclose(f); }
#define MYDEBUGINT(a, b) { FILE *f = fopen("d:\\kryss\\debug.txt", "a+t"); fprintf(f, "%s %d\n", a, b); fclose(f); }
#define MYDEBUGFLOAT(a, b) { FILE *f = fopen("d:\\kryss\\debug.txt", "a+t"); fprintf(f, "%s %f\n", a, b); fclose(f); }
...
void CSmartFitterClass::MergeTracks2(Track2 *pTracks, int Count)
{
	...
	DX = T.RecoveryDeltaPos;
	DY = T.RecoveryDeltaPos;
	CellCountX = floor(WX / DX + 5.);
	CellCountY = floor(WY / DY + 5.);
	CellCount = CellCountX * CellCountY;
