...
BEGIN_MESSAGE_MAP(CMainSwitch, CDialog)
	...
	ON_BN_CLICKED(IDC_MS_FOGTHICKNESS, &CMainSwitch::OnFogThickness)
	ON_BN_CLICKED(IDC_MS_IMAGEDUMP, &CMainSwitch::OnImageDump)
END_MESSAGE_MAP()
...
void CMainSwitch::OnFogThickness()
{
	// TODO: Add your control notification handler code here
	m_Selection = MS_SEL_FOGTHICKNESS;
	OnCancel();		
}
...
BOOL CMainSwitch::OnInitDialog() 
{
	...
	*phWnd = m_hWnd;
	...
	GetDlgItem(IDC_MS_IMAGEDUMP)->EnableWindow(m_EnableScan);
...
void CMainSwitch::OnImageDump()
{
	m_Selection = MS_SEL_IMAGEDUMP;
	OnCancel();		
}
