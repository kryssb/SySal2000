...
CScan::CScan(
	...
	: CDialog(CScan::IDD, pParent), phWnd(phwnd), CVertigoScanner(iself, iio, ist, iob, 
		...
		ipc2, //GS
		c), Terminate(false), 
		ForcedName(forcedname), RemoteScan(remotescan), AutoZReset(autozreset), TrustZ(trustz), pDataConsumer(0), pDataConsumerContext(0)
#ifdef ENABLE_NET
...
CScan::CScan(
	...
	: CDialog(CScan::IDD, pParent), phWnd(phwnd), CVertigoScanner(iself, iio, ist, iob, 
		...
		ipc2,  //GS
		c), Terminate(false), 
		ForcedName(0), RemoteScan(true), AutoZReset(autozreset), TrustZ(trustz), pDataConsumer(0), pDataConsumerContext(0)
#ifdef ENABLE_NET
		,TkClient(tkclient), m_NPFrag(nullptr)
#endif
{
...
void CScan::InstallDataConsumer(void *pctx, CScan::DataConsumer dc)
{
	pDataConsumerContext = pctx;
	pDataConsumer = dc;
}
...
UINT CScan::Scan(float startx, float starty, bool xforward, bool yforward, bool lastfragment)
{
	if (SingleViewMode)
...
UINT CScan::Thread()
{
	...
	for (CurrentFragment = 1; !ShouldStop() && (CurrentFragment <= (XFragments * YFragments)); CurrentFragment++)
	{
		...
		DUMPTIME(10);
		Error = Scan(StartX, StartY, (2 * (CurrX - StartX) / C.XStep) < C.XFields(), (2 * (CurrY - StartY) / C.YStep) < C.YFields(), CurrentFragment == (XFragments * YFragments));
		DUMPTIME(11);
		CHECK_AUTOZ("Z1F");
		if (ShouldStop())
		{
	...
	catch (System::Exception ^x)
	{
		...
		MakeDebugReport("jhydx", x, ssxs);
	}
	catch (...)
	{
		::MessageBox(0, "C++ exception", "Exception in jhydx", MB_OK);
	}
...
/*
	...
	iSt->GetPos(1, &Y);
*/
	return VSSCAN_OK; //return CVertigoScanner::GetFocus(X, Y, CurrentRelevantZs);
	};
...
static char duplicate_milestone[32];

#define DUPLICATE_MILESTONE(u) { strcpy(duplicate_milestone, u); }
...
{	
	...
	if (!System::Object::ReferenceEquals(TkClient, nullptr) && C.EnableScanGrid && !SingleViewMode)
	{
		...
		DUPLICATE_MILESTONE("F")
		int i;
		for (i = 0; i < Frag.Fragment.CountOfViews; i++)
		{
			DUPLICATE_MILESTONE("G")
			...
			DUPLICATE_MILESTONE("H")
			};

		DUPLICATE_MILESTONE("I")
		return pFrag;
	}
#endif
	DUPLICATE_MILESTONE("J")
	IO_VS_Fragment2 *pFrag = (IO_VS_Fragment2 *)malloc(sizeof(IO_VS_Fragment2));
	DUPLICATE_MILESTONE("K")
	pFrag->Hdr = Frag.Hdr;
	memcpy(pFrag->Reserved, Frag.Reserved, sizeof(Frag.Reserved));
	pFrag->Fragment.CodingMode = Frag.Fragment.CodingMode;
	pFrag->Fragment.CountOfViews = Frag.Fragment.CountOfViews;
	...
	catch (System::Exception ^ssx)
	{
		...
		ssxs += "Milestone ";
		ssxs += gcnew System::String(duplicate_milestone);
		ssxs += "\r\n";
		ssxs += "i " + i.ToString() + "\r\n";
		ssxs += "v " + v.ToString() + "\r\n";
		ssxs += "s " + s.ToString() + "\r\n";
		try
		{
			ssxs += "NPFrag " + m_NPFrag->Index.ToString() + " length " + m_NPFrag->ViewInfo->Length + "\r\n";
			for (h = 0; h < m_NPFrag->ViewInfo->Length; h++)
			{
				ssxs += "View " + h.ToString() + " " + m_NPFrag->ViewInfo[h]->IsTop.ToString();
				ssxs += " bytes: " + m_NPFrag->ViewInfo[h]->ClusterLayers->Length + "\r\n";
			}
		}
		catch (System::Exception ^) {}
		MakeDebugReport("ddxww", ssx, ssxs);
		return 0;
	}
}
...
UINT CScan::DataWriteThread()
{
	...
	{
		System::String ^xstr = x->ToString();
		char *tempstr = new char[xstr->Length + 1];
		int i;
		for (i = 0; i < xstr->Length; i++)
			tempstr[i]= xstr[i];
		tempstr[i] = 0;
		::MessageBox(0, tempstr, "Exception in cscdwrthr", MB_OK);
		delete [] tempstr;
	}
	catch (...)
	{
		::MessageBox(0, "C++ exception", "Exception in cscdwrthr", MB_OK);
	}
	return 0;
}
...
void CScan::EnqueueDataToWrite(DataToWrite d)
{
	if (pDataConsumer) 
	{
		pDataConsumer(pDataConsumerContext, &d);
