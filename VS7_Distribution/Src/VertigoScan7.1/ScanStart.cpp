...
CScanStart::CScanStart(CWnd* pParent /*=NULL*/, bool takeimages)
...
void CScanStart::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_SS_IMDUMPFILE, m_ImageDumpFile);
	...
	DDX_Text(pDX, IDC_SS_IMDUMPX, m_ImDumpX);
	DDX_Text(pDX, IDC_SS_IMDUMPY, m_ImDumpY);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CScanStart, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_SS_SELIMDUMPFILE, &CScanStart::OnBnClickedSsSelimdumpfile)
END_MESSAGE_MAP()
...
void CScanStart::EnableGroups(int index)
{
	...
	GetDlgItem(IDC_SS_ID1)->ShowWindow(NewBatch || ImageDump);
	GetDlgItem(IDC_SS_ID2)->ShowWindow(NewBatch || ImageDump);
	GetDlgItem(IDC_SS_ID3)->ShowWindow(NewBatch || ImageDump);
	GetDlgItem(IDC_SS_ID4)->ShowWindow(NewBatch || ImageDump);
	GetDlgItem(IDC_SS_STATIC_BATCHID)->ShowWindow(NewBatch || ImageDump);
	...
	GetDlgItem(IDC_SS_SLOPEX)->ShowWindow(NewBatch || ImageDump);
	GetDlgItem(IDC_SS_SLOPEACCX)->ShowWindow(NewBatch);
	GetDlgItem(IDC_SS_SLOPEY)->ShowWindow(NewBatch || ImageDump);
	GetDlgItem(IDC_SS_SLOPEACCY)->ShowWindow(NewBatch);
	GetDlgItem(IDC_SS_STATIC_SLOPEX)->ShowWindow(NewBatch || ImageDump);
	GetDlgItem(IDC_SS_STATIC_SLOPEACCX)->ShowWindow(NewBatch);
	GetDlgItem(IDC_SS_STATIC_SLOPEY)->ShowWindow(NewBatch || ImageDump);
	GetDlgItem(IDC_SS_STATIC_SLOPEACCY)->ShowWindow(NewBatch);
	GetDlgItem(IDC_SS_SELIMDUMPFILE)->ShowWindow(ImageDump);
	GetDlgItem(IDC_SS_IMDUMPFILE)->ShowWindow(ImageDump);
	GetDlgItem(IDC_SS_IMDUMPX)->ShowWindow(ImageDump);
	GetDlgItem(IDC_SS_IMDUMPY)->ShowWindow(ImageDump);
	GetDlgItem(IDC_SS_STATIC_IMDUMPX)->ShowWindow(ImageDump);
	GetDlgItem(IDC_SS_STATIC_IMDUMPY)->ShowWindow(ImageDump);
}

void CScanStart::OnBnClickedSsSelimdumpfile()
{
	CFileDialog fdlg(FALSE, 0, 0, OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT, "Image files (*.mic)|*.mic|Text files (*.txt)|*.txt|All files (*.*)|*.*", this);
	if (fdlg.DoModal() == IDOK)
	{
		m_ImageDumpFile = fdlg.GetPathName();
		UpdateData(FALSE);
	}
}
