...
CScan::CScan(HWND *phwnd, ISySalObject *iself, ISySalDataIO *iio, IStage *ist,
...
UINT CScan::Thread()
{
	UINT ExtErrorInfo;
	CString MsgString;
	if (iIO->Write((HSySalHANDLE)m_hWnd, (BYTE *)&Cat, &ExtErrorInfo, 0) != S_OK)
	{
...
UINT CScan::GetRelevantZs(VS_View::t_RelevantZs &F, UINT field, bool istop)
{
	switch (ValidCorners)
	{
...
UINT CScan::PrepareFocusInfo()
{
	...
	for (i = 0; i < 4; i++)
	{
		if ((!pCorners[i]->TakenOnFragment) || ((CurrentFragment - pCorners[i]->TakenOnFragment) > 1))
		{
			...
			VS_View::t_RelevantZs F = pCorners[i]->RelevantZs;
