...
CVertigoScanner::CVertigoScanner(ISySalObject *iself, ISySalDataIO *iio, IStage *ist,
	IObjective *iob, IFrameGrabber *ifg, IVisionProcessor *ivp, ITracker *itk1, ITrackFitter *ift1, 
	ITracker *itk2, ITrackFitter *ift2, ISheetMap *ism, ConfigData &c)
	: iIO(iio), iSt(ist), iOb(iob), iFG(ifg), iVP(ivp), iSM(ism), C(c), iSelf(iself), UseVP2(false)
{
	iTk[0] = itk1;
	iTk[1] = itk2;
	iFt[0] = ift1;
...
UINT CVertigoScanner::Init(bool bAllocateClusterSpace)
{
	...
	try
	{
		...
		if (!(pTSpace = (Track *)GlobalAlloc(GMEM_FIXED, 2 * sizeof(Track) * C.MaxTracks))) throw 2;
		if (!(pPSpace = (TVector *)GlobalAlloc(GMEM_FIXED, 2 * sizeof(TVector) * C.MaxTracks * C.VLayers))) throw 3;
		UINT FitCorrDataSize = Frag.Fragment.FitCorrectionDataSize = 0;
		iFt[0]->GetCorrectionDataSize((UINT *)&FitCorrDataSize);
		iFt[1]->GetCorrectionDataSize((UINT *)&Frag.Fragment.FitCorrectionDataSize);
		if (Frag.Fragment.FitCorrectionDataSize < FitCorrDataSize) Frag.Fragment.FitCorrectionDataSize = FitCorrDataSize;
			if (!(pCorrSpace = (BYTE *)GlobalAlloc(GMEM_FIXED, 2 * Frag.Fragment.FitCorrectionDataSize * C.MaxTracks))) throw 4;
		
		for (i = 0; i < 2; i++)
			for (j = 0; j < C.MaxTracks; j++)
			{
				pTSpace[i * C.MaxTracks + j].pPoints = pPSpace + (i * C.MaxTracks + j) * C.VLayers;
				if (Frag.Fragment.FitCorrectionDataSize)
					pTSpace[i * C.MaxTracks + j].pCorrection = pCorrSpace + (i * C.MaxTracks + j) * Frag.Fragment.FitCorrectionDataSize;
				};
		ProcessVariables.pRec = (t_ProcessVariables::t_Rec *)((BYTE *)pCSpace + sizeof(Cluster) * C.MaxClusters);

		if (!(pVSpace = (VS_View *)GlobalAlloc(GMEM_FIXED, sizeof(VS_View) * C.XFields * C.YFields))) throw 5;
		if (!(pZSpace = (float *)GlobalAlloc(GMEM_FIXED, 2 * sizeof(float) * C.XFields * C.YFields * C.VLayers))) throw 6;

		Frag.Fragment.pViews = pVSpace;
		for (i = 0; i < C.XFields * C.YFields; i++)
			for (j = 0; j < 2; j++)
				pVSpace[i].Layers[j].pZs = pZSpace + (2 * i + j) * C.VLayers;
		}
	catch (int x)
	{
...
// NOTE: No stop on Z...
		...
		for (ProcessVariables.Layer = 0; ProcessVariables.Layer < C.VLayers && !ZLimiter; ProcessVariables.Layer++)
		{
			...
			else ProcessVariables.pRec[++v].Z = ProcessVariables.Z;
			CHECK_STOP;
			};
...
////////////// END NEW FOCUSING
		...
		for (l = 0; l < C.VLayers; l++)
			Frag.Fragment.pViews[ProcessVariables.Field].Layers[s].pZs[l] = ProcessVariables.pRec[l].Z;
		...
		iTk[s]->GetTracks(pTSpace + ProcessVariables.TCount[s] + s * C.MaxTracks, &MaxCount, 0, C.VLayers - 1);
		...
		iFt[s]->Fit(pTSpace + ProcessVariables.TCount[s] + s * C.MaxTracks, &MaxCount, 
...
UINT CVertigoScanner::DiscardInvalidTracks(Track *pTracks, UINT Count)
{
	UINT i, j;
	for (i = j = 0; i < Count; i++)
	{
		if (pTracks[i].Valid)
		{
			...
			pTracks[j].InterceptErrors = pTracks[i].InterceptErrors;
			pTracks[j].LastZ = pTracks[i].LastZ;
			pTracks[j].PointsN = pTracks[i].PointsN;
			pTracks[j].Sigma = pTracks[i].Sigma;
			pTracks[j].Slope = pTracks[i].Slope;
			pTracks[j].SlopeErrors = pTracks[i].SlopeErrors;
			memcpy(pTracks[j].pPoints, pTracks[i].pPoints, sizeof(TVector) * pTracks[i].PointsN);
...
void CVertigoScanner::ZExpand(Track *pTracks, UINT Count, float ZBase)
{
	UINT i, j;
	for (i = 0; i < Count; i++)
		for (j = 0; j < pTracks[i].PointsN; j++)
			pTracks[i].pPoints[j].Z = ((pTracks[i].pPoints[j].Z - ZBase) * C.Shrinkage + ZBase) * CSP.RefractiveShrinkage;
}

void CVertigoScanner::XYOffset(Track *pTracks, UINT Count)
{
	UINT i, j;
	for (i = 0; i < Count; i++)
		for (j = 0; j < pTracks[i].PointsN; j++)
		{
			pTracks[i].pPoints[j].X -= ViewCenterX;
			pTracks[i].pPoints[j].Y -= ViewCenterY;
			};
}

UINT CVertigoScanner::GetFocus(float TargetX, float TargetY, VS_View::t_RelevantZs &F)
{
	bool XLimiter, YLimiter, ZLimiter;
	bool InEmulsion;
	int Lim, l, s;
...
UINT CVertigoScanner::GetRelevantZs(VS_View::t_RelevantZs &F, UINT field, bool istop)
{
