...
//		ProcessVariables.TargetZ -= C.VTol;
		...
		for (ProcessVariables.Layer = 0; ProcessVariables.Layer < C.VLayers && !ZLimiter; ProcessVariables.Layer++)
		{
			...
			if (ProcessVariables.Layer >= FrameDelay)
			{
				...
				iTk[0]->PutClusters(pCSpace, ProcessVariables.CCount, ProcessVariables.pRec[v - FrameDelay].Z);
				if (ProcessVariables.pTempCSpace) memcpy(pCSpace, ProcessVariables.pTempCSpace + C.MaxClusters * (v - FrameDelay), sizeof(Cluster) * ProcessVariables.CCount);
				iTk[1]->PutClusters(pCSpace, ProcessVariables.CCount, ProcessVariables.pRec[v - FrameDelay].Z);
...
////////////// END NEW FOCUSING
		...
		for (ss = 0; ss < 2; ss++)
		{
			MaxCounts[ss] = C.MaxTracks - ProcessVariables.TCount[ss];
			iTk[ss]->GetTracks2(pTSpace + ss * C.MaxTracks, &MaxCounts[ss], 0, C.VLayers - 1);
			for (i = 0; i < MaxCounts[ss]; i++) 
			{
				pTSpace[i + ss * C.MaxTracks].Field = ProcessVariables.Field;
				};
			}
		{
			...
			{
				char temp[256];
				sprintf(temp, "Different numbers of tracks found! %d vs. %d", MaxCounts[0], MaxCounts[1]);
				MessageBox(0, temp, "Debug Info", MB_OK);
				}
			else
			{
				MaxCount = MaxCounts[0];
				int hh, jj;
				for (hh = 0; hh < MaxCount; hh++)
				{
					if (pTSpace[hh].Grains != pTSpace[hh + C.MaxTracks].Grains)
					{
						MessageBox(0, "Different numbers of grains found!", "Debug Info", MB_OK);
						break;
						}
					else
					{
						for (jj = 0; jj < pTSpace[hh].Grains; jj++)
						{
							if (pTSpace[hh].pGrains[jj].X != pTSpace[hh + C.MaxTracks].pGrains[jj].X ||
								pTSpace[hh].pGrains[jj].Y != pTSpace[hh + C.MaxTracks].pGrains[jj].Y ||
								pTSpace[hh].pGrains[jj].Z != pTSpace[hh + C.MaxTracks].pGrains[jj].Z /* ||
								pTSpace[hh].pGrains[jj].Area != pTSpace[hh + C.MaxTracks].pGrains[jj].Area*/)
							{
								MessageBox(0, "Different grains found!", "Debug Info", MB_OK);
								break;
								}
							}
						if (jj < pTSpace[hh].Grains) break;
						}
					}
				}
			MaxCount = 0;
