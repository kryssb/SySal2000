...
UINT CVertigoScanner::Init()
{
	...
	try
	{
		...
		for (i = 0; i < (1 + ProcessVariables.FieldDelay); i++)
		{
			...
			if (!(ProcessVariables.pAIS[i].pImages = (AsyncProcessedImage *)GlobalAlloc(GMEM_FIXED, sizeof(AsyncProcessedImage) * ProcessVariables.pAIS[i].ImageCount))) throw 10;
			for (j = 0; j < ProcessVariables.pAIS[i].ImageCount; j++) ProcessVariables.pAIS[i].pImages[j].pClusters = 0;
			if (!(ProcessVariables.pAIS[i].pImages[0].pClusters = (Cluster *)GlobalAlloc(GMEM_FIXED, sizeof(Cluster) * C.MaxClusters * ProcessVariables.pAIS[i].ImageCount))) throw 11;
...
UINT CVertigoScanner::ScanSingleView(float startx, float starty, bool moveafterscan, float moveafterx, float moveaftery, bool retry)
{
	...
	int firstlayer, lastlayer, adjlayers, s, smplindex, cthresh;
	float zadj = 0.0f;
...
*/				if (s == 0)
		...
		switch (C.FocusLockModel)
		{
			...
			case FOCUS_LOCK_CENTERLEVEL:
				{
					int centerlevel = (firstlayer + lastlayer) / 2;
			...
			case FOCUS_LOCK_CENTERVALLEY:
				{
					int valleypos = FindValley(ProcessVariables.pAIS[s], firstlayer, lastlayer);
