...
namespace SySal
{
	namespace DAQSystem
	{
			...
			try
			{
					...
					m_Frag->SlopePresetMinY, m_Frag->SlopePresetMaxY))				
				{
					...
					for (i = 0; i < m_Frag->ViewInfo->Length; i++)
					{
						...
						if (m_Server->SendViewSide(nw->Field, nw->TileX, nw->TileY, nw->PosX, nw->PosX, nw->MapPosX, nw->MapPosY, nw->MapXX, nw->MapXY, nw->MapYX, nw->MapYY, nw->Z, nw->ClusterLayers, nw->IsTop, nw->ZTop, nw->ZBottom, nw->ZBase, nw->Shrinkage) == false) break;
					}
				}
		...
		void TrackingClientConnection::Execute()
		{
			...
			HANDLE HEvs[2];
			HEvs[0] = HDataQueueMutex;
			HEvs[1] = HTerminate;
			System::Int64 SrvGUID = 0;
			while (WaitForMultipleObjects(2, HEvs, FALSE, 1000) != WAIT_OBJECT_0 + 1)
			{				
				try
				{
					if (m_Server == nullptr)
...
*/					
		...
		void TrackingClient::AddFragment(NetPackagedFragment ^pkfg)
		{
			...
			{				
				WaitForSingleObject(HDataQueueMutex, INFINITE);	
				if (m_DataQueue->Count < 2/* + this->m_Servers->Length*/)
				{
