...
void CVertigoScan6Class::Answer(System::Threading::AutoResetEvent ^hSS, System::Object ^obj)
{
...
Object ^CVertigoScan6Class::Request()
{
	...
	if (hSS != nullptr)
	{
		...
		{			
			...
			else if (reqobj->GetType() == SySal::DAQSystem::Scanning::ZoneDesc::typeid || (reqobj->GetType() == System::Collections::ArrayList::typeid && ((System::Collections::ArrayList ^)reqobj)[0]->GetType() == SySal::DAQSystem::Scanning::ZoneDesc::typeid))
			{
				...
				Ret = Scan.ResetAndScan(Id, zone->MinX, zone->MaxX, zone->MinY, zone->MaxY, 0, txtout, TrustZ, true, 
			...
			else if (reqobj->GetType() == SySal::DAQSystem::Scanning::PlateQuality::FogThicknessSet::typeid)
			{
				...
				float zref;
				bool ok = ExecGetFogThickness(iSt, iFG, fgthks, zref);
				System::AppDomain::CurrentDomain->SetData("$_IsBusy_$", nullptr);
				Answer(hSS, fgthks);
			}
			else
	...
	else
	{
		...
		do
		{
			...
			case MS_SEL_TESTCONFIG:
				{
					CTestAndConfig TestAndConfig(&m_hDefWnd, (ISySalObject *)this, iIO, iSt, iOb, iFG, iVP, iTk1, iFt1, iTk2, iFt2, iSM, &CMI, C);
			...
			case MS_SEL_FOGTHICKNESS:
				SySal::DAQSystem::Scanning::PlateQuality::FogThicknessSet fgthks;
				float zref = 0.0f;
				if (!ExecGetFogThickness(iSt, iFG, fgthks, zref)) ::MessageBox(0, "Fog/Thickness measurement failed.", "Measurement error.", MB_ICONERROR);
				else
				{
					CPlateQualityDlg pq;
...
bool CVertigoScan6Class::ExecGetFogThickness(IStage2 *iSt, IFrameGrabber *iFG, SySal::DAQSystem::Scanning::PlateQuality::FogThicknessSet &fgthks, float &zreference)
{
#define MAXREFS 31
	...
	for (i = 0; i < xyrefcount; i++)
	{
		...
		iSt->PosMove(0, xyrefs[i * 2], C.HSpeed, C.HAcc);
		iSt->PosMove(1, xyrefs[i * 2 + 1], C.HSpeed, C.HAcc);
		...
		while (fabs(w - xyrefs[i * 2]) > C.HTol + 10.0);
		iSt->Stop(0);
		do
		{
			iSt->GetPos(1, &w);
		}
		while (fabs(w - xyrefs[i * 2 + 1]) > C.HTol + 10.0);
		iSt->Stop(1);
		do
		{
			iSt->GetPos(2, &w);
...
float CVertigoScan6Class::FindMedian(float *px, int stride, int count)
{
	int place, scan, best;
...
bool CVertigoScan6Class::ExecLoadPlate(float *pAutoZReset, IStage *iSt, IFrameGrabber *iFG, IPlateChanger *iPc, Identifier id, char *textid)
{
	ISySalObject *iStOb = 0;
	iSt->QueryInterface(IID_ISySalObject, (void **)&iStOb);
	iSt->Stop(0);
...
//End Robot Related
	...
	if (Ret == IDOK)
	{
		...
		if (C.AutoTuneFog)
		{
			...
			{
				if (ExecGetFogThickness(iSt2, iFG, fgthks, *pAutoZReset))
				{
					C.TunedTopStep = fgthks.TopThickness;
					C.TunedBottomStep = fgthks.BottomThickness;
				}
				iSt2->Release();
				iSt2 = 0;
			}
