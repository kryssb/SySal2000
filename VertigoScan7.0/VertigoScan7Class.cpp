	...
	if (hSS != nullptr)
	{
		...
		{			
			...
			else if (reqobj->GetType() == SySal::DAQSystem::Scanning::ZoneDesc::typeid || (reqobj->GetType() == System::Collections::ArrayList::typeid && ((System::Collections::ArrayList ^)reqobj)[0]->GetType() == SySal::DAQSystem::Scanning::ZoneDesc::typeid))
			{
				...
				Ret = Scan.ResetAndScan(Id, zone->MinX, zone->MaxX, zone->MinY, zone->MaxY, 0, txtout, TrustZ, zone->UsePresetSlope/*true*/, 
...
bool CVertigoScan7Class::ExecGetFogThickness(IStage2 *iSt, IFrameGrabber *iFG, SySal::DAQSystem::Scanning::PlateQuality::FogThicknessSet &fgthks, float &zreference)
{
#define MAXREFS 31
...
//Robot Related
	...
	{   
        ...
        //if(!LDres) return false;                                                              //GS   commented by GS: non standard return value
