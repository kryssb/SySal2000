...
{
	protected:
		static DWORD *pCorruptionFlag;
		static DWORD CorruptionTest;
		static void SetCorruptionFlag(DWORD *p);
		static void CheckCorruption(char *s);
		...
		float *pSafetyTopIntZ;		
		int GrainMultiplicity;
		bool TracksPageGrains;
		...
		struct t_ProcessVariables
		{
			...
			bool StartZInited;
            VS_View2::t_RelevantZs2 StartZs;
			} ProcessVariables;
		...
		virtual UINT ScanSingleView(float startx, float starty, bool moveafterscan = false, float moveafterx = 0.0f, float moveaftery = 0.0f, bool retry = false);
		...
		void MakeDebugReport(char *location, System::Exception ^x, System::String ^addinfo);
	...
	public:
        ...
        virtual void NotifyPlateLoaded();
};
