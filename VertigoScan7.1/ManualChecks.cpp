...
BEGIN_MESSAGE_MAP(CManualChecks, CDialog)
	...
	ON_WM_TIMER()
	ON_BN_CLICKED(IDCANCEL, &CManualChecks::OnBnClickedCancel)
END_MESSAGE_MAP()
...
void CManualChecks::OnFlagSel() 
{
	...
	if (m_Auto)
	{
		if (m_Found == TRUE)
		{
			...
			{
				if (MessageBox("Position out of tolerance in X.\r\nAccept anyway?", "Incorrect candidate", MB_YESNO | MB_DEFBUTTON2) == IDNO)
					return;
				}
			if (fabs(m_IY - First.Y) > m_PosTol) 
			{
				if (MessageBox("Position out of tolerance in Y.\r\nAccept anyway?", "Incorrect candidate", MB_YESNO | MB_DEFBUTTON2) == IDNO)
					return;
				}
			if (fabs(m_SX - Slope.X) > m_SlopeTol) 
			{
				if (MessageBox("Slope out of tolerance in X.\r\nAccept anyway?", "Incorrect candidate", MB_YESNO | MB_DEFBUTTON2) == IDNO)
					return;
				}
			if (fabs(m_SY - Slope.Y) > m_SlopeTol) 
			{
				if (MessageBox("Slope out of tolerance in Y.\r\nAccept anyway?", "Incorrect candidate", MB_YESNO | MB_DEFBUTTON2) == IDNO)
					return;
				}
			}
		EndDialog(IDOK);
...
void CManualChecks::OnBnClickedCancel()
{
	// TODO: Add your control notification handler code here
	OnCancel();
}
