...
#define MILESTONE Milestone = gcnew System::String(__FILE__   " "  __FUNCTION__ " " ) + System::Int32(__LINE__).ToString();

namespace SySal
{
	namespace DAQSystem
	{
		void TrackingServer::Test(System::String ^testfile)
		{
			System::IO::StreamWriter ^w = nullptr;
			try
			{
				w = gcnew System::IO::StreamWriter(testfile);
				w->Write("VS ScanGrid test");
				w->Flush();
			}
			finally
			{
				if (System::Object::ReferenceEquals(w, nullptr) == false)
				{
					w->Close();
					w = nullptr;
				}
	...
	void SySal::DAQSystem::TrackingServer::SafeNotifyError(System::String ^err)
	{
		...
		if (NotifyError != nullptr) NotifyError(err + gcnew System::String("\r\n") + Milestone);
	...
	{				
		...
		try
		{
			...
			MILESTONE
			pTracker_T->EnableSlopePresetting(useslopepreset);
			pTracker_B->EnableSlopePresetting(useslopepreset);
			pTracker_T->PresetSlope(0.5 * (slopepresetminx + slopepresetmaxx), 0.5 * (slopepresetminy + slopepresetmaxy), 
				0.5 * (slopepresetmaxx - slopepresetminx), 0.5 * (slopepresetmaxy - slopepresetminy));
			pTracker_B->PresetSlope(0.5 * (slopepresetminx + slopepresetmaxx), 0.5 * (slopepresetminy + slopepresetmaxy), 
				0.5 * (slopepresetmaxx - slopepresetminx), 0.5 * (slopepresetmaxy - slopepresetminy));				
			MILESTONE
			return true;
		}
