	...
	C(c), iSelf(iself), LightSet(false)
{
    ...
    ProcessVariables.StartZInited = false;
    ProcessVariables.StartZs.TopExt = ProcessVariables.StartZs.TopInt = ProcessVariables.StartZs.BottomExt = ProcessVariables.StartZs.BottomInt = 0.0f;
...
*/
    ...
    if (!ProcessVariables.StartZInited)
    {
        UINT FocusResult = GetRelevantZs(ProcessVariables.StartZs, 0, ProcessVariables.IsTop);
/*		if (FocusResult != VSSCAN_OK) 
		{
			return FocusResult;
                        };
*/      ProcessVariables.StartZInited = true;   
        };
    RelZ = ProcessVariables.StartZs;
...
*/
	...
	while (Loop /*ProcessVariables.Field < (C.XFields() * C.YFields()) || fieldflushcount < ProcessVariables.FieldDelay*/)
	{
/*		FILE *fhist = fopen("c:\\acq\\vs6.1.hist.txt", "a+t");
...
*/		
		...
		if (fieldprechargecount > ProcessVariables.FieldDelay)
		{
			...
			Milestone = 29;
			if (firstlayer >= lastlayer || (ProcessVariables.ValidLayers < C.MinValidLayers && (firstlayer == C.VInactiveLayers || lastlayer == C.VLayers - 1)))
			{
				Milestone = 30;
				bool LastField = false;
				{
					int a;
					bool b;
					LastField = !GetNextView(f, s == 0, a, b);
					}
				Milestone = 31;
				if (!Retry && !LastField && GetFocus(ProcessVariables.X, ProcessVariables.Y, RelZ) == VSSCAN_OK)
				{
					NextField = f;
					NextIsTop = (s == 0);
					iSt->WaitForAsyncMotionComplete((DWORD)ShouldStopCallback, (DWORD)this);
				...
				if ((int)(lastlayer + 1 - firstlayer) >= (int)C.MinValidLayers)
				{
					...
					if (C.FocusLockModel == FOCUS_LOCK_CENTERVALLEY)
					{
						...
						else 
						{
							...
							if (s == 0)
							{
								ProcessVariables.StartZs.TopExt = RelZ.TopExt = vpz + C.VStep(s) * 0.5; 
                                ProcessVariables.StartZs.TopInt = RelZ.TopInt = vpz - C.VStep(s) * 0.5;
                                }
							else
                            {
								ProcessVariables.StartZs.BottomInt = RelZ.BottomInt = vpz + C.VStep(s) * 0.5;   
                                ProcessVariables.StartZs.BottomExt = RelZ.BottomExt = vpz - C.VStep(s) * 0.5;
                                }
...
void CVertigoScanner::NotifyPlateLoaded()
{
        ProcessVariables.StartZInited = false;
}
